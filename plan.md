...
- [ ] Provisioning recovery yüzeyini tamamla: explicit failed-step retry ve durable compensation API/state mevcut; Unix identity ile Nginx operation-owned rollback/compensation kullanıyor. Kalan mutating adapter'lara aynı ownership/evidence sözleşmesini yay, restart sonrası interrupted apply/compensation reconcile-resume akışını ve step-level remediation UI/UX'ini tamamla. Evidence/ownership olmadan başarı veya destructive cleanup üretme; host mutation'ı körlemesine tekrar etme.
