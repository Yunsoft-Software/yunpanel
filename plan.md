...
- [ ] Legacy Website path/runtime/SFTP migration apply/rollback kapsamını tamamla; Unix identity, SFTP, Passenger Node, PHP ve static runtime exact bounded preview artık var. Mutation yalnız current preview digest'i hâlâ geçerliyken ve adapter ownership evidence operation-owned kaynağı kanıtlarken açılsın; kör recursive `chown`/`rm` yapılmasın.
