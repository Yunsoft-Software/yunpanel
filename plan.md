...
- [ ] Direct-systemd → Passenger control-plane zincirini tamamla. Host tarafında read-only preview, canonical env binding, health-gated Nginx cutover, systemd cleanup ve rollback coordinator mevcut; protocol/local-operation contract, authenticated canonical preview ve durable runtime-binding registry de mevcut. Kalan iş: `app.node.passenger-migrate` operation'ını ana job queue/result sanitizer zincirine tam bağla; apply endpoint'i yalnız persisted Application/Website/Domain snapshot'ından job üretsin; başarılı veya `cleanup_required` sonucu exact Website/Domain/release revision'larına bağlı `ApplicationRuntimeBinding` olarak reconcile edilsin; Passenger binding varken normal Domain restage eski proxy/systemd target'ına dönememesin; interrupted cleanup/reconciliation restart-safe tamamlanabilsin. Bir Website'e birden fazla bağımsız Domain route bağlıysa tek-route cutover açılmasın.
- [ ] Passenger runtime binding'i Domain/Nginx target resolution'ın canonical authority'sine bağla. Runtime binding yalnız host health evidence + operation ownership + current Website/Domain/release revision eşleşmesiyle yazılsın; revision drift'te fail-closed reconciliation bıraksın. Runtime binding yoksa persisted legacy Node uygulama `direct-systemd` compatibility olarak yorumlanabilsin; yeni Website için `direct-systemd` varsayılanı üretme.
