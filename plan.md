...
## 2026-09-22 ürün kararı — site yaşam döngüsü ve arayüz (Antigravity uygulama brifi)

Kullanıcının bu turdaki açık talimatı, daha önce ertelenmiş görsel UI/UX çalışmasını **şimdi** kapsama alır. Aşağıdaki işler tamamlanmış kabul edilmez; kaynak değişikliği, güvenlik testleri ve `.28` üzerinde gerçek tarayıcı/host kabulü ayrı izlenir. `agents.md` içindeki main, küçük commit, veri/secret koruması ve `.44` yasağı geçerlidir. Önce mevcut bileşenleri ve durable provisioning/job sözleşmesini kullan; ikinci bir yönetim/kimlik doğrulama veya Roundcube kurulumu üretme. Tasarım yalnız JSX/CSS, TypeScript yok; browser `prompt`/`alert` yok.

### P0 — Veritabanı kapsamı ve site yöneticisi

- Roundcube altyapı şemaları (`roundcube`, `roundcube_*`, `roundcubemail`, `roundcubemail_*`, büyük/küçük harf varyantları) normal veritabanı envanteri, sayı/boyut, Website seçicisi ve kullanıcıya dönük yedek listesinde görünmez. API ve host mutasyon sınırı bu adlar için create/bind/delete/backup/restore/grant yollarını reddeder; mevcut şemalar yerinde kalır. Bu kod diliminin canlı tarayıcı ve doğrudan API kabulü `todo.md` T-DB-UI'dadır.
- Yeni **bağımsız ana Website** formu yönetici e-postası ve yeni parola ister; backend'de Website kimliğine tekil site-admin bağı, Argon2id hash, oturum/site kapsamı ve güvenli migration uygulanır. Bu kişi mail kutusu, YunPanel Owner ve MySQL kullanıcısı değildir. Alt alan adı/shared-site için yönetici kimliği üst Website ile ilişkilendirilir; bağımsız subdomain davranışı, mevcut site izolasyonunu zedelemeden açıkça modellenir. Var olan siteler için kontrollü davet/reset ile veri migrasyonu yapılır; tahmin edilen veya varsayılan parola üretilmez.
- Site-admin yalnız kendi Website kaynaklarını görür ve kendi DB şema/kullanıcı yetkisi, parola rotasyonu ve phpMyAdmin erişimini yönetebilir. Owner bütün site DB'lerinde tam yetkilidir. Bütün API/job/gateway/WebSocket yollarında gerçek backend authorization ve site scope test edilir; UI gizlemek yetki sayılmaz. Bir Website yaşarken bağlı site-admin silme/devre dışı bırakma reddedilir; Owner yalnız e-posta/parola değişimini güvenli yeniden doğrulama/audit ile yapabilir. Site silme bağımlılık ve retained veri sözleşmesiyle birlikte yönetici bağını kaldırır.
- Site oluşturma veritabanı kurmaz. Siteye DB ancak kullanıcı sonradan açık create eylemiyle eklenir; MySQL localhost kullanıcısı/grant/şifresi site-admin oturumundan bağımsız, secret store'da saklanan ayrı credential'dır. Website DB ve global Owner DB satırında yetkili, Website-scoped phpMyAdmin handoff düğmesi bulunur; doğrudan root veya başka site şeması açılmaz. Parola görünür metin/URL/job/audit'e düşmez.

### P0 — Ana Website için otomatik DNS, mail ve webmail

- Yeni bağımsız **ana domain** Website'inde local DNS zone, mail domain ve shared Roundcube `webmail.<domain>` mapping varsayılan zorunlu provisioning adımlarıdır; subdomain/alias için yeni zone/mail domain/Roundcube instance yaratılmaz. Sunucu başına paylaşılan Roundcube kurulumunun varlığı ve sağlığı doğrulanır; yoksa dependency/package job'ı ve certificate/DNS/webmail bağlama adımları health-gated yürür. Registrar delegasyonu veya dış DNS otomatik değişmiş gibi gösterilmez. Eksik DNS kimliği/servis/sertifika için baştan blocker, adım hatasında açık partial/failed durum gösterilir; yanlışlıkla `ready` yazılmaz. Mevcut site/default davranışını migration olmadan topluca değiştirme.
- Create önizlemesi Unix user, runtime, Nginx, DNS, mail, webmail ve sertifika hedeflerini gösterir. Durable operation her adımın `queued/running/succeeded/failed/blocked` durumunu, güvenli hata nedenini, tamamlanma sayısını ve yeniden girişte son halini verir; UI gerçek job/provisioning kaydını izler. Retry yalnız kullanıcı eylemiyle, kaldığı güvenli noktadan idempotent evidence/compensation ile yapılır. Üçüncü başarısız denemede otomatik tekrar durur (`retry_exhausted`); manuel yeniden dene olanağı sürer. Bunu global kör job iptali olarak uygulama: her operation'ın retry policy, lock ve crash recovery kontratı korunur.

### P0 — Panel hesabı ve SSL iletişimi

- Ayarlar > Panel yöneticisi bölümünde Owner kurtarma e-posta adresi gerçek kullanıcı kaydına bağlı, doğrulanmış ve güncellenebilir olur. Parolamı unuttum, yalnız bu adrese süreli/tek kullanımlık reset bağlantısı yollar; token hash olarak saklanır, URL log/audit'e yazılmaz, eski oturumlar iptal edilir, rate limit/anti-enumeration/MFA policy korunur. SMTP yoksa gönderilmiş gibi gösterme. Site-admin e-postası bu genel Owner kurtarma adresi değildir.
- SSL oluşturma formu açıldığında etkin authenticated kullanıcının e-postası ACME iletişim alanına önceden doldurulur; kullanıcı açıkça değiştirebilir. Panel genel ACME varsayılanı ve parola kurtarma adresi birbirine bağlanmaz.

### P1 — Arayüz ve bilgi mimarisi (görsel çalışma artık kapsamda)

- Ayarlar, üst/yan kategori sekmeleri ve deep-link ile yalnız **değiştirilebilir** politika/formları gösterir: Panel hesabı, Site varsayılanları, DNS/SSL, Mail, Veritabanları, Güvenlik, Yedek ve Entegrasyonlar. Statik çalışma modu, mimari açıklamalar, dosya yolu ve sürüm envanterini Ayarlar'dan çıkar; gerekli teşhis/versiyonları Sunucu > Tanılama'ya taşı. `Yerel yönetim (Agentless Root)` son kullanıcı yüzeyinde gösterilmez. `[object Object]` asla render edilmez; bilinmeyen servis durumu açıkça bilinmeyen kalır, API unit nesnesi `name/state` alanlarıyla doğru gösterilir. Sunucu servisleri tek otorite sayfasında; tekrarlar Ayarlar'dan kalkar.
- İşlemler (job geçmişi/progress) ve Denetim menü bağlantıları günlük sidebar'dan çıkarılır; Ayarlar > İşlemler ve kayıtlar / Tanılama üzerinden bulunur, gerekli global işlem kısayolu/progress çekmecesi korunur. Terminal ve servisler yalnız Sunucu bağlamında bulunur; yinelenen navigation temizlenir. Loglar ana sayfada büyük tablo doldurmaz, ilgili site/Sunucu tanılama bağlantısı veya Ayarlar altındaki erişim noktasından okunur. Yetki/scope ve link/deep-link bozulmaz.
- Genel bakışta Website listesi yerine sayı ve ilgili listeye geçiş göster; CPU/RAM/disk kullanımını yüzdeyle orantılı, erişilebilir metin etiketli halka göstergelerinde sun (reduced-motion desteği, bilinmeyen veri için boş/unknown durum; 91% disk gibi yüksek doluluğa uygun uyarı). Yakın SSL, hata ve aktif işler için küçük, eyleme dönük özetler kullan. Mobil/klavye/ekran okuyucu ve açık/koyu tema kabulü yapılır. Mevcut reusable bileşen/tokens ile tutarlı görsel hiyerarşi kur; sahte animasyon veya metrik üretme.
...
Son fresh Website dedicated webmail certificate provisioning ilerlemesi: `docs/history/site-create-webmail-certificate-provisioning-2026-09-19.md`.
Son fresh Website local-mail health-gate ilerlemesi: `docs/history/site-create-mail-health-gate-2026-09-19.md`.
Son fresh Website mail discovery source ilerlemesi: `docs/history/site-create-mail-discovery-source-2026-09-19.md`.
...
Kaynakta yeni managed-mail preview/apply yolu SQLite-backed virtual domain/mailbox/alias/quota/password-hash read-model'ine geçirildi. Control-plane registries source-of-truth kalır; private SQL seed deterministic state digest'iyle durable mail apply/backup/rollback zincirine girer. Postfix domain/mailbox/alias/sender-login lookup'ları `proxy:sqlite`, Dovecot auth `passdb sql` + dedicated `vmail` static userdb kullanır. Maildir `vmail:vmail` altında Website UID'den ayrı tutulur; SQL DB ayrı `root:yunpanel-mailauth` state root'unda yalnız Postfix+Dovecot reader grubuna açılır. Managed service/package policy `postfix-sqlite/dovecot-sqlite/sqlite3` bağımlılıklarını taşır. Apply SQL DB quick-check + state digest + directory/file ownership + config/reload/health kanıtından sonra eski hash/passwd live lookup'larını kaldırır. Backup manifest v6 SQL ve legacy state'i snapshot'lar, v5 okuyucu ve eski digest materialization compatibility upgrade recovery için korunur. SMTP 25 (no SASL, plain auth kapalı), submission 587 (`smtpd_tls_security_level=encrypt`, `smtpd_tls_auth_only=yes`), SMTPS 465 (`smtpd_tls_wrappermode=yes`, `smtpd_tls_security_level=encrypt`), IMAP 143 (STARTTLS) ve IMAPS 993 (`ssl=yes`), Dovecot `disable_plaintext_auth = yes` ve `ssl = required` policy'si kaynak kodda ve 5'li protocol health inspector'da (`smtp:25`, `submission:587`, `submissions:465`, `imap:143`, `imaps:993`) tamamlanmıştır (bkz. [mail-smtp-submission-tls-policy-2026-09-20.md](file:///Users/yunsoftmac/Desktop/Projeler/yunpanel/docs/history/mail-smtp-submission-tls-policy-2026-09-20.md)). Sender-login mapping alias desteği (hem hash hem SQLite UNION ALL), anti-open-relay (`smtpd_recipient_restrictions`), strict HELO/EHLO (`smtpd_helo_required`), Postfix anvil rate limiting (bağlantı, mesaj, alıcı, TLS hız sınırları) ve Rspamd Milter `{auth_authen}` makro entegrasyonu tamamlandı (bkz. [mail-sender-login-relay-abuse-policy-2026-09-20.md](file:///Users/yunsoftmac/Desktop/Projeler/yunpanel/docs/history/mail-sender-login-relay-abuse-policy-2026-09-20.md)). Forwarding (Sieve), alias (virtual_aliases), SRS (PostSRSd) yaşam döngüleri, posta kutusu silmede takma ad referansı bağımlılık kontrolü (`mailbox_delete_alias_reference_configured`) ve yönlendirme-takma ad çapraz döngü engellemesi (`assertNoMailRoutingCycles`) tamamlandı (bkz. [mail-forwarding-alias-srs-lifecycle-2026-09-20.md](file:///Users/yunsoftmac/Desktop/Projeler/yunpanel/docs/history/mail-forwarding-alias-srs-lifecycle-2026-09-20.md)). ClamAV isteğe bağlı profili (`antivirusProfile: 'disabled' | 'clamav'`), Rspamd `/etc/rspamd/local.d/antivirus.inc` entegrasyonu, `mail-antivirus-health-inspector` sağlık denetimi (`dpkg-query`, `systemctl is-active`, `/run/clamav/clamd.ctl` soketi ve 1GB+ bellek kontrolü), `mail-readiness-inspector` kanonik ClamAV gereksinimleri (`clamav`) ve sağlık yoksa aktif göstermeme (`active: false`) kontratı kaynak kodda tamamlandı (bkz. [mail-clamav-optional-profile-health-2026-09-20.md](file:///Users/yunsoftmac/Desktop/Projeler/yunpanel/docs/history/mail-clamav-optional-profile-health-2026-09-20.md)). Node 24 kaynak testleri ve full check geçmiştir; gerçek Ubuntu SMTP/IMAP/LMTP canlı kabulü 2026-09-20'de tamamlanmıştır (bkz. [mail-smtp-imap-live-acceptance-2026-09-20.md](file:///Users/yunsoftmac/Desktop/Projeler/yunpanel/docs/history/mail-smtp-imap-live-acceptance-2026-09-20.md)). Fresh Website local mail, dedicated webmail TLS sertifikası, shared Roundcube mapping, mailbox Roundcube oturumu ve Mail Domain disable webmail mapping blocker/teardown `.28` üzerinde doğrulanmıştır (bkz. [site-create-local-mail-roundcube-live-acceptance-2026-09-20.md](file:///Users/yunsoftmac/Desktop/Projeler/yunpanel/docs/history/site-create-local-mail-roundcube-live-acceptance-2026-09-20.md), [mail-domain-disable-webmail-blocker-live-acceptance-2026-09-20.md](file:///Users/yunsoftmac/Desktop/Projeler/yunpanel/docs/history/mail-domain-disable-webmail-blocker-live-acceptance-2026-09-20.md)). Failure-injection kabulü `todo.md` T-MAIL altında devam eder.
Gerçek inbound/outbound SMTP, IMAP, Roundcube ve anti-abuse kabul kapıları `todo.md` içindedir.

## P0.6 — Yerli İzolasyonlu Dosya Yöneticisi ve Otomatik Webmail Entegrasyonu

Kaynak kod tarafında ve canlı test sunucusunda (`157.180.11.28`) yerli dosya yöneticisi (`site-file-worker.js`, `site-file-manager.js`, `FilesPanel.jsx`) sitenin Linux kullanıcısı (`website.unixUser`) altında izole olarak çalıştırıldı ve tüm dosya operasyonları (mkdir, create_file, read_text, write_text, rename, batch_delete, list) kabul testlerinden geçti. `webmail.<domain>` Nginx konfigürasyonu, Let's Encrypt SSL sertifikasyonu ve HTTP->HTTPS yönlendirmesi canlı sunucuda (`webrich.news`) otomatik olarak doğrulanıp tamamlandı. DNS sekmesinde alt alan adlarının üst PowerDNS zone'una bağlanması ve kayıt yönetimi `DnsPanel.jsx` üzerinde hayata geçirildi. Gerçekleştirilen adımların ayrıntıları `docs/history/native-file-manager-and-auto-webmail-live-acceptance-2026-09-21.md` içindedir.
...
1. **Mail core parity** — authenticated SMTP/IMAP, TLS port policy, DKIM/SPF/DMARC, relay/rate abuse/Rspamd, forwarding/SRS, Mail Domain disable ve external-DNS lifecycle'ındaki açık kaynak işlerini tamamla.
