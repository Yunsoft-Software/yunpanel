...
- [ ] Kalan host adapter'larını mevcut canonical Website/Application identity/path contract'ına bağla. Static production caller, SFTP, logs/analytics ve backup scope aynı `yunapp-*`, `/var/lib/yunpanel/data/<applicationId>`, release/current ve control-plane backup authority bilgisini tüketmeli; path/user formülünü adapter içinde yeniden üretme ve drift'i fail-closed bırak.
...
- [ ] Mevcut `WebsiteStaticDeploymentManager`ı production `APP_STATIC_DEPLOY` yoluna Website-aware identity binding ile bağla. Deploy kendi Unix user'ını yaratamasın; canonical HOME/group drift'inde fail-closed olsun; mevcut build/publish/release/rollback ve legacy persisted job compatibility davranışını koru.
