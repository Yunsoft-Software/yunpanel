...
- [ ] `MAIL_CONFIG_ROLLBACK` için mevcut v3 rollback preview'ın operation/backup/current-state-bound typed confirmation'ını doğrulayan durable job ekle; restore başlamadan current active-config evidence'ını doğrula, restart'ta inspect-first mixed-state/receipt recovery yap ve başarılı rollback sonrası mail-domain status'unu receipt'teki previous status'a monoton revision ile reconcile et.
