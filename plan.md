...
- [ ] Local mail provisioning'in discovery ve gerçek-host acceptance kapılarını tamamla. Deterministic Mail Domain metadata reservation, operation-owned route `purpose: web` certificate issue/attachment + `tls_activation`, zero-mailbox `mail_config`, deterministic `mail_dkim_key`, authoritative `mail_dns_reapply`, ayrı operation-owned `purpose: webmail` ACME certificate issuance/reconciliation, DNS-ready-gated `mail_dkim_config`, shared Roundcube `roundcube_mapping` ve required `mail_health` source gate'i durable Website provisioning journal'ında bağlıdır. `mail_health` current mail config/readiness + SMTP 25/submission 587/IMAP 143 listener + exact Roundcube HTTPS endpoint evidence'ı olmadan Website'i ready yapmaz. Eksik kısım autodiscover/autoconfig ile gerçek Ubuntu mailbox-auth, STARTTLS, inbound/outbound ve browser acceptance'tır.
...
4. **TLS/autodiscover/recovery hardening** ve gerçek-host cross-service/mailbox-auth acceptance kapıları.
