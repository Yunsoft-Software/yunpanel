...
- [ ] Mevcut delete impact graph'ında henüz unavailable/eksik kalan Unix/runtime/DB/SFTP/log bağımlılık provider'larını tamamla; persisted general-backup operation/artifact evidence'ından application/database/Docker/mail ilişkilerini çıkaran `backups` provider'ı ve canlı `/etc/cron.d` host envanteri ile `websiteCronRegistry` arasında çift taraflı SHA-256 ve orphan denetimi yapan fail-closed `crons` provider'ı production parent preview ve normal impact API'ına bağlandı. Site-user cron tarafında Website execution identity'sine pinli durable registry, ortak 5-field cron/template contract'ı, root-owned `/etc/cron.d` inspect/apply/remove + salt okunur managed-file host envanteri ve production registry bootstrap kaynakta hazırdır. Sıradaki işler registry→host mutation için durable job/reconciliation ve kalan eksik impact provider'larıdır. Parent removal preview gerekli dependency provider'ların tamamını operation intent'ine pinlemeden public apply açma.
...
4. **TLS/recovery hardening** ve gerçek-host cross-service/mailbox-auth/mail-client acceptance kapıları.
