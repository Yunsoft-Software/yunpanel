...
- [ ] Ek Owner oluşturma, kullanıcı düzenleme/devre dışı bırakma ve gerekiyorsa kaynak bazlı Read Only rolünü backend + UI ile geliştir. Son aktif Owner silinememeli/devre dışı bırakılamamalı; rol değişimi mevcut oturumlarda etkili olmalı. Read Only için mevcut kapalı yönetim sınırını yalnızca açık kaynak/işlem izinleriyle genişlet. Yeni yönetici rotalarını kendi hesap ayarları istisnasına koyma; mevcut Owner MFA yönetim kontrolünden geçir.
...
- [ ] WebSocket/SSE/terminal eklendiğinde HTTP ile aynı oturum/rol/Origin ve Owner MFA yönetim kontrolünü kur; loopback geliştirme istisnasını gelecekteki public root/terminal yoluna taşıma. Logout, parola/MFA/rol değişimi ve kullanıcı iptalinde canlı bağlantı/PTY yetkisini derhal kaldır. Şimdilik kapalı upgrade yolunu korumasız açma.
...
**Kabul:** Gerçek HTTPS, tam Node 24 workspace ve tarayıcı kabulü `todo.md` T1/T1a/T1b/T1c üzerinden tamamlanmalı. Mevcut HTTPS MFA kuralının gerçek host kabulü, kullanıcı yaşam döngüsü ve ileride canlı bağlantı iptali doğrulanmadan root/terminal public sürümü açılmamalı. Kodun test edilmesi canlı deployment kanıtı sayılmayacak.
