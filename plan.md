...
- [ ] Static deploy queue/payload'ını Website kimliğine explicit bağla; canonical-first host router üzerindeki proven-legacy fallback yalnız persisted migration işleri için tanınsın ve real-host acceptance sonrası kaldırılsın. Operation-owned release inspect/rollback ile Website binding revision'ı birlikte doğrulansın; yeni unbound deploy legacy yoluna düşmesin.
