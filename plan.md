...
- [ ] Domain removal parent journal'ındaki kalan webmail/mail/external-DNS step handler'larını tamamla. Hazır deepest-first child operation → certificate registry retirement → Website binding detach → authoritative DNS retirement → metadata finalization zincirini yalnız önceki dependency evidence tamamlandıktan sonra çalıştır; mail/external-DNS için gerçek destructive lifecycle olmadan step'i başarılı sayma ve standalone authoritative DNS delete route'u açma.
