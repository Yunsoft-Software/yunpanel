...
- [ ] Local-mail domain -> `webmail.<domain>` DNS/TLS/Nginx -> shared Roundcube mapping'inin provisioning acceptance'ını tamamla. Mevcut Mail Domain için source lifecycle hazır: hostname-covering active certificate fence'i, revisioned pending/active/removing/removed mapping registry, shared Nginx/FPM desired state, exact successful Roundcube apply health evidence sonrası DNS gate ve typed bind/delete continuation vardır.
...
- [ ] Local mail provisioning'i shared Roundcube/cross-service health ile finalize et. Deterministic Mail Domain metadata reservation, operation-owned route certificate issue/attachment + `tls_activation`, zero-mailbox `mail_config`, deterministic `mail_dkim_key`, authoritative `mail_dns_reapply` ve DNS-ready-gated `mail_dkim_config` durable Website provisioning journal'ında kaynakta bağlıdır. Eksik kısım `webmail.<domain>` certificate coverage + shared Roundcube bind/apply, discovery/autoconfig ve final SMTP/IMAP/webmail health evidence'ıdır.
