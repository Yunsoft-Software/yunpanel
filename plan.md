...
- [ ] Provisioning recovery contract'ını kalan adapter'lara yay: explicit failed-step retry, durable compensation API/state, secret-safe latest-operation read model, Site overview step-level continue/retry/compensate UI, Unix identity + Nginx operation-owned rollback/compensation, startup `listInterrupted()` inspect-first reconcile ve handler-kind bazlı actionable remediation guidance mevcut. Static runtime apply/inspect de deterministic release ve inspect-first restart yoluna bağlı; kalan mutating adapter'lar ile static runtime compensation aynı ownership/evidence/recovery sözleşmesini kullanmalı. Evidence/ownership olmadan başarı veya destructive cleanup üretme, restart sonrası host mutation'ı körlemesine tekrar etme.
- [ ] Kalan host adapter'larını mevcut canonical Website/Application identity/path contract'ına bağla. Global static production caller, SFTP, logs/analytics ve backup scope aynı `yunapp-*`, `/var/lib/yunpanel/data/<applicationId>`, release/current ve control-plane backup authority bilgisini tüketmeli; path/user formülünü adapter içinde yeniden üretme ve drift'i fail-closed bırak.
...
- [ ] Global/legacy `APP_STATIC_DEPLOY` yolunu Website-aware identity contract'a migration-safe biçimde bağla ve static provisioning compensation/rollback ownership ekle. Yeni Website deploy'u kendi Unix user'ını yaratmasın, canonical HOME/group drift'inde fail-closed kalsın; mevcut build/publish/release/rollback ve legacy persisted job compatibility davranışı korunsun.
