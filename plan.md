...
- [ ] Domain removal parent journal'ındaki kalan step handler'larını tamamla: deepest-first child Domain operations → certificate/webmail/mail/external-DNS cleanup → authoritative DNS retirement. Hazır Website binding detach ve metadata finalization step'lerini yalnız önceki dependency evidence tamamlandıktan sonra çalıştır; certificate/mail/external-DNS için gerçek destructive lifecycle olmadan step'i başarılı sayma ve standalone authoritative DNS delete route'u açma.
