...
- [ ] Domain removal parent journal'ındaki kalan webmail/external-DNS lifecycle'larını tamamla. Mail Domain child tarafında config-disable → metadata cleanup → verified backup → mailbox credential cleanup → backup-bound data delete → local/external finalization phase router + production bootstrap kaynakta bağlandı; gerçek shared Roundcube `webmail.<domain>` mapping lifecycle'ı oluştuğunda yalnız operation-owned mapping cleanup'ını ekle. External DNS için provider ownership sınırını aşmayan destructive retirement/unlink handler'ını kur. Hazır deepest-first child operation → certificate registry retirement → Website binding detach → authoritative DNS retirement → metadata finalization zincirini yalnız önceki dependency evidence tamamlandıktan sonra çalıştır; gerçek external-DNS lifecycle olmadan step'i başarılı sayma ve standalone authoritative DNS delete route'u açma.
