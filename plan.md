...
- [ ] Domain removal parent journal'ındaki kalan webmail/mail/external-DNS lifecycle'larını tamamla. Hazır durable Mail Domain child registry/runtime/cleanup plan/config-disable ve pinned alias/quota/forwarding/DKIM metadata-cleanup adapter'ları üzerine mailbox credential cleanup, verified backup-bound data delete ve finalization phase adapter'larını, external akış için provider ownership sınırını aşmayan metadata unlink adapter'ını kur; domain-başına gerçek bir Roundcube mapping lifecycle'ı tanımlanmadan sahte cleanup üretme. Phase router'ı ve runtime'ı production bootstrap'ta parent `mail_domain` handler sözleşmesine bağla. Hazır deepest-first child operation → certificate registry retirement → Website binding detach → authoritative DNS retirement → metadata finalization zincirini yalnız önceki dependency evidence tamamlandıktan sonra çalıştır; gerçek child/external-DNS lifecycle olmadan step'i başarılı sayma ve standalone authoritative DNS delete route'u açma.
