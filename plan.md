...
- [ ] Local mail provisioning'i cross-service health ile finalize et. Deterministic Mail Domain metadata reservation, operation-owned route `purpose: web` certificate issue/attachment + `tls_activation`, zero-mailbox `mail_config`, deterministic `mail_dkim_key`, authoritative `mail_dns_reapply`, ayrı operation-owned `purpose: webmail` ACME certificate issuance/reconciliation, DNS-ready-gated `mail_dkim_config` ve shared Roundcube `roundcube_mapping` durable Website provisioning journal'ında kaynakta bağlıdır. Eksik kısım discovery/autoconfig ve final SMTP/IMAP/webmail health evidence'ıdır.
