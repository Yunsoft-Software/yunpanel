...
- [ ] Domain removal parent journal'ındaki kalan step handler'larını tamamla: descendant'a ait local authoritative-DNS preview/ownership/retention intent'ini parent plana pinleyip parent-owned child operation'a aktar → certificate/webmail/mail/external-DNS cleanup. Hazır deepest-first child operation → Website binding detach → authoritative DNS retirement → metadata finalization zincirini yalnız önceki dependency evidence tamamlandıktan sonra çalıştır; certificate/mail/external-DNS için gerçek destructive lifecycle olmadan step'i başarılı sayma ve standalone authoritative DNS delete route'u açma.
