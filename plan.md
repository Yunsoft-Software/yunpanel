...
- [ ] Global/legacy `APP_STATIC_DEPLOY` caller'ını mevcut Website-aware static deployment manager'a migration-safe bağla. Legacy persisted job compatibility, canonical HOME/group drift fail-closed davranışı ve operation-owned release inspect/rollback sözleşmesi korunsun; global caller ikinci Unix identity veya ayrı path formülü üretmesin.
