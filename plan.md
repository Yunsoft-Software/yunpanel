...
- [ ] Local-mail domain -> `webmail.<domain>` DNS/TLS/Nginx -> shared Roundcube mapping'inin provisioning acceptance'ını tamamla. Mevcut Mail Domain lifecycle'a ek olarak fresh Website source journal artık required `roundcube_mapping → webmail_dns_reapply` zincirini, exact operation ownership'i, inspect-first reconciliation'ı ve inspect-proven explicit retry semantiğini taşır. Kalan kaynak işi dedicated `purpose: webmail` certificate selection/issuance + Roundcube evidence bağını tamamlamaktır; gerçek Ubuntu/Roundcube/DNS acceptance ayrıca `todo.md` içindedir.
...
- [ ] Local mail provisioning'i dedicated webmail certificate/cross-service health ile finalize et. Deterministic Mail Domain metadata reservation, operation-owned route `purpose: web` certificate issue/attachment + `tls_activation`, zero-mailbox `mail_config`, deterministic `mail_dkim_key`, authoritative `mail_dns_reapply`, DNS-ready-gated `mail_dkim_config`, shared Roundcube `roundcube_mapping` ve ardından `webmail_dns_reapply` durable Website provisioning journal'ında kaynakta bağlıdır. Eksik kısım ayrı `purpose: webmail` certificate selection/issuance, discovery/autoconfig ve final SMTP/IMAP/webmail health evidence'ıdır.
- [ ] Fresh local-mail Website için `purpose: webmail` certificate lifecycle'ını ayrı durable provisioning step olarak bağla: ana Domain `purpose: web` selection'ını değiştirmeden `webmail.<domain>` hostname'ini issue/reconcile et, Roundcube mapping yalnız bu exact operation-owned webmail certificate evidence'ını kabul etsin ve restart/lost-ack inspect ikinci ACME mutation üretmesin.
