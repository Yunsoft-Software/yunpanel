...
- [ ] Passenger Website runtime readiness/inspect akışını canonical Website/Application identity inspector'ına bağla. `satisfied:true` dönmeden önce `yunapp-*` user/group, UID/GID, nologin shell, HOME=`/var/lib/yunpanel/data/<applicationId>`, HOME ownership/mode=`0750` ve path-bound identity state yeniden doğrulansın; identity missing/drift durumunda Passenger/Node/release sağlıklı olsa bile runtime hazır sayılmasın. Apply sonrası postcondition aynı identity kontrolünü tekrar çalıştırsın ve ikinci bir user/path formülü üretmesin.
