...
- [ ] Mevcut strict `MAIL_CONFIG_ROLLBACK` protocol/local-executor/job-result zincirine root-private rollback receipt ve v3 rollback preview typed-confirmation enqueue yüzeyi ekle; restart'ta inspect-first source/current/mixed-state recovery yap ve başarılı rollback sonrası mail-domain status'unu receipt'teki previous status'a monoton revision ile reconcile et.
