...
- [ ] Host mutation sınırına bağlı mevcut strict `MAIL_CONFIG_ROLLBACK` protocol/local-executor/job-result/root-private journal+receipt zincirine v3 rollback preview typed-confirmation enqueue yüzeyi ekle; restart'ta inspect-first source/current/mixed-state recovery yap ve başarılı rollback sonrası mail-domain status'unu receipt'teki previous status'a monoton revision ile reconcile et.
