...
Before release, complete the remaining real HTTPS/MFA/Read Only/audit checks under `todo.md` T-AUTH-AUDIT, the migration/rollback checks under T-MIGRATION and package/browser gates under T-PACKAGE-LIVE and T-FEATURE-ACCEPTANCE. The limitations above describe this historical increment and must not be read as the current repository-wide test result. Existing network restrictions remain in force; no GitHub Actions were added or used.
