...
The phpMyAdmin template bundle pins PHP 8.3 to the `yunpanel-phpmyadmin` runtime identity, `/run/php/yunpanel-phpmyadmin.sock`, separate runtime-private temp/session directories and hardened session/PHP flags. Its Nginx server has no TCP listener: it listens only on `/run/yunpanel/phpmyadmin-http.sock`, blocks `setup`, `test`, `libraries`, `templates` and hidden paths, and publishes the intended `root:yunpanel-web 0660` socket contract for the future same-origin gateway. Template preview is not activation evidence; the durable activator must still validate FPM/Nginx, enforce directory/socket metadata, reload safely and prove endpoint health.
