...
phpMyAdmin follows the same package-only boundary with the exact `phpmyadmin`, `php-fpm` and `php-mysql` package set. Inspection checks only the fixed `/usr/share/phpmyadmin/index.php` and `/etc/phpmyadmin/config.inc.php` paths plus PHP syntax, reports no fabricated systemd unit, and does not imply that a protected Nginx/FPM endpoint or YunPanel signon handoff exists. Package install completion and restart recovery accept this unitless state through the managed-service policy; the systemd control route rejects phpMyAdmin.

The phpMyAdmin template bundle pins PHP 8.3 to the `yunpanel-phpmyadmin` runtime identity, `/run/php/yunpanel-phpmyadmin.sock`, separate runtime-private temp/session directories and hardened session/PHP flags. Debian post-install creates this locked service account, grants only the package-config `www-data` supplementary group and prepares its home, temp and session directories as identity-owned `0700`. Its Nginx server has no TCP listener: it listens only on `/run/yunpanel/phpmyadmin-http.sock`, blocks `setup`, `test`, `libraries`, `templates` and hidden paths, and publishes the intended `root:yunpanel 0660` socket contract matching the packaged same-origin gateway identity. Exact previews may be written only to digest-scoped `0700` staging directories as `0640` non-symlink artifacts; byte, mode, identity metadata or symlink drift fails inspection. Staging is not activation evidence; the durable activator must still validate FPM/Nginx, enforce runtime directory/socket metadata, reload safely and prove endpoint health.

Before live phpMyAdmin activation, the host backup manager records the exact previous FPM/Nginx presence, bytes, checksum, uid, gid and mode in a transaction-scoped `0700` directory with `0600` artifacts and a strict manifest. Restore atomically replaces prior files and removes only targets proven absent before the transaction. Symlink live targets, duplicate transactions, expanded manifests and changed backup bytes fail closed. This supplies rollback material but does not itself mutate or validate the active services.
