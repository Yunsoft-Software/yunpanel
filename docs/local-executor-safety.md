...
GitHub webhook secrets use a separate reserved record in the same master-key-encrypted store and are never returned by management reads or materialized into a hosted process. The public endpoint verifies the exact raw body with `X-Hub-Signature-256` before interpreting payload fields. Its delivery UUID is retained only as private job idempotency state; public job and audit metadata omit it. Enqueue atomically rejects another queued/running job for the same resource, while an exact delivery replay resolves to the already durable job.
