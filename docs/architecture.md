...
- Website isolation audit migration gerektiren her metadata/operation/step farkını current ve desired değerlerle ayrı değişiklik olarak verir; provisioning intent'in kendisini açmadan SHA-256 kimliğini pinler. Preview digest bu exact değişiklikleri ve canlı inspection reason'ını kapsar, typed confirmation buna bağlanır. Canonical Unix identity legacy preview UID/GID, account HOME/shell, private-group member count ve HOME mode drift'ini secret-safe biçimde pinler; desired user/HOME yeniden canonical Application identity ile doğrulanır. SFTP drift preview da aynı digest'e bound edilir. Tek fark canonical workspace `tmp`/`logs` eksikliği olduğunda authenticated apply; durable migration journal'a önce intent yazar, current preview digest'i yeniden doğrular ve operation-scoped receipt ile yalnız eksik direct-child dizinleri oluşturur. Restart kör mutation replay etmez; receipt inspection tamamlanmış apply'i kapatır, belirsiz apply açık kalır. Typed rollback yalnız receipt-owned boş dizinleri non-recursive kaldırır. Unix identity safe-create, runtime veya SFTP geniş migration apply yolu data-preserving compensation/ownership sözleşmesi tamamlanana kadar fail-closed kalır.
