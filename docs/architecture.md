...
- Website isolation audit migration gerektiren her metadata/operation/step farkını current ve desired değerlerle ayrı değişiklik olarak verir; provisioning intent'in kendisini açmadan SHA-256 kimliğini pinler. Preview digest bu exact değişiklikleri ve canlı inspection reason'ını kapsar, typed confirmation buna bağlanır. Canonical Unix identity preview UID/GID, account HOME/shell, private-group member count ve HOME mode drift'ini; SFTP preview receipt/config/mount/chroot/key-materialization state'ini; Passenger preview shared runtime/Node binary/current-release/UMask evidence'ını; PHP preview release-container ownership + FPM package/pool/socket/UMask evidence'ını; static preview publish/releases ownership+ACL/current-symlink evidence'ını bounded biçimde aynı digest'e bağlar. Durable isolation migration journalı beş dar operation-owned aileyi destekler: eksik `tmp`/`logs` workspace direct-child'ları, tamamen eksik canonical Unix identity, tamamen eksik site-specific SFTP isolation, shared PHP-FPM runtime/container/UMask zaten canonical iken yalnız eksik site pool ve yalnız PHP control-plane `applicationRoot`/`releasesDirectory`/`current` metadata drift'i. Son PHP container ailesi exact source provisioning/release ID'sini ayrı pinler; release/public content site UID/GID `0750`, current target exact, site FPM runtime sağlıklı ve shared `UMask=0027` olmadan açılmaz. Receipt mutation öncesi previous UID/GID/mode'u yazar; apply/rollback yalnız üç exact control-plane path'te non-recursive metadata mutation yapar, path type veya foreign metadata drift'inde fail-closed kalır. Her apply current revision+digest'i yeniden doğrular, restartta receipt/host inspection ile tamamlanmış mutation'ı kapatır ve typed rollback yalnız receipt-owned state'i geri alır. Passenger legacy runtime ve static runtime preview'ları `automaticMigration=false` kalır. Passenger için bu flag isolation journal mutation'ı olmadığını belirtir: Site overview canonical Application Passenger migration workflow'una handoff yapar; source route/release/environment snapshot'ı, Nginx checksum cutover/rollback, target health, legacy systemd cleanup ve `ApplicationRuntimeBinding` reconciliation orada yönetilir. Static tarafta isolation journal artık yalnız control-plane `publishRoot`, `releasesRoot` ve `current` metadata drift'i için `static_control` adapter'ına authority verir: release tree ve Nginx ACL'ler preview/apply/restart inspection sırasında tamamen sağlıklı olmak zorundadır; receipt previous UID/GID/mode ile current target'ı mutation öncesi pinler ve apply/rollback yalnız üç exact path'te non-recursive metadata değiştirir. Managed release file/directory ownership-mode veya Nginx ACL drift'i bu adapter kapsamı dışındadır ve halen blocked kalır. Daha geniş PHP release/content/path drift'i de operation-owned exact rollback authority dışında fail-closed kalır.
