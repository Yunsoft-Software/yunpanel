...
- Database ekranı her normal GET/refresh'te yerel MySQL/MariaDB Unix socket'inden strict-sanitize edilmiş canlı envanter okur; schema satırları persisted Website binding ve secret-free credential durumuyla birleştirilir, eşleşmeyen ownership state fail-closed kalır. Günlük akış “sunucuyu tara” job'ına bağlı değildir. Create/drop/credential/backup/restore mutation'ları durable job, binding ve ownership kanıtı sınırında kalır. Shared phpMyAdmin/pgAdmin gateway henüz yoktur.
