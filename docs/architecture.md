...
Website genel bakışındaki izolasyon denetimi canonical Unix kullanıcısı ve yolları durable provisioning operation + canlı handler inspection kanıtıyla karşılaştırır. Bu yüzey salt okunurdur; migration gerektiğinde exact-change preview/digest ve typed confirmation üretir fakat apply orchestration ile tüm adapter rollback ownership sözleşmesi tamamlanmadan host mutation başlatmaz.
