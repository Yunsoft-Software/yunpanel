...
- DNS tarafında external lifecycle/Cloudflare adapter'ına ek olarak PowerDNS Authoritative package/config/service adapter'ı, encrypted API-key store, server NS identity, versioned zone template, Website local-DNS provisioning, RRset yönetimi, durable re-apply, DNSSEC ve secondary/delegation gözlemi kaynakta vardır. Authoritative durable journal'ın current/latest state, inspect evidence ve automatic-replay-blocked recovery nedeni Network DNS panelinde görünür; explicit retry/rollback/resolve kontrolü ile gerçek Ubuntu, public UDP/TCP 53, delegation/glue, transfer/failover ve registrar DNSSEC kabulü tamamlanmadan production-ready sayılmaz.
