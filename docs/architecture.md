...
- Managed mail desired-state geçişi `MAIL_CONFIG_APPLY` durable job'ıyla local single-writer executor'a bağlıdır. Executor protected desired material'ı public job kaydına taşımadan materialize eder; stage sonrasında pre-apply backup alır, exact configuration/plan/backup digest bağını doğrular ve ancak bundan sonra validator, atomic activation/reload ve readiness kapılarından geçer. Başarılı bounded job result'ı ile root-private operation receipt v2 aynı `configurationSha256`, `planSha256`, `backupSha256` ve `readinessSha256` zincirini taşır. Restart/lost-ack recovery exact receipt, yeniden materialize edilen desired config ve canlı host evidence'ı eşleşmeden job'u kapatmaz; mutation'ı otomatik replay etmez. Legacy v1 receipt recovery için okunur fakat backup identity içermediğinden explicit rollback yetkisi vermez. Backup-bound `MAIL_CONFIG_ROLLBACK`, current-state fence, restart-safe compensation ve control-plane reconciliation hâlâ açık iştir; gerçek Postfix/Dovecot/Rspamd host kabulü geçmeden production-ready sayılmaz.
