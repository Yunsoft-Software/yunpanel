...
Website genel bakışındaki izolasyon denetimi canonical Unix kullanıcısı ve yolları durable provisioning operation + canlı handler inspection kanıtıyla karşılaştırır. Denetim salt okunurdur ve migration gerektiğinde exact-change preview/digest ile typed confirmation üretir. Apply yalnız canonical Unix identity zaten sağlıklı, tek drift eksik `tmp`/`logs` workspace direct-child'ları ve durable receipt rollback'i hazır olduğunda açılır; diğer drift türleri ilgili adapter ownership sözleşmesi tamamlanana kadar mutation başlatmaz.
