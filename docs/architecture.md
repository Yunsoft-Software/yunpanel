...
- DNSSEC rollover için güncel yürütme notu: propagation inspector production control-plane'e bağlanmıştır. Journal v3 exact key-set/serial/new-key identity ile DNS identity target'larını yeniden doğrular; primary ve bütün secondary uçlarda exact DNSKEY görüldükten ve en yüksek TTL dolduktan sonra bounded propagation evidence ile exact activation target digest'ini `activating_key` intent'ine persist eder. Hazır olmayan gözlem host mutation yapmaz. Activation mutation'ı, parent DS geçişi ve old-key cleanup halen açık iştir; bu not önceki paragraftaki eski propagation-wiring durumunun yerini alır.
