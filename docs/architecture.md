...
- DNSSEC rollover için güncel yürütme notu: propagation inspector production control-plane'e bağlanmıştır. Journal v3 exact key-set/serial/new-key identity ile DNS identity target'larını yeniden doğrular; primary ve bütün secondary uçlarda exact DNSKEY görüldükten ve en yüksek TTL dolduktan sonra bounded propagation evidence ile exact activation target digest'ini `activating_key` intent'ine persist eder. Hazır olmayan gözlem host mutation yapmaz. Activation current/target digest ile lost-ack-safe yürür. Parent'ta önce clean old+new double-DS, sonra yalnız new-key DS ve foreign DS yokluğu doğrulanır; parent inspector v3 ayrıca en uzun authoritative DS RRset TTL'ini verir. Bu TTL'in old-DS yokluk gözleminden itibaren durable bekleme kapısına bağlanması ve old-key deactivate/delete cleanup halen açık iştir. Bu not önceki paragraftaki eski propagation-wiring durumunun yerini alır.
