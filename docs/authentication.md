...
Back up configuration/state and confirm independent SSH/provider-console access before changing a host. Preserve the existing `YUNPANEL_SECRET_MASTER_KEY` and configure it in the API environment before upgrading: MFA enrollment needs this key when enabled, and application secrets still depend on it when Owner MFA is optional. Do not replace a working key merely to satisfy the prerequisite. When an intentional key rotation is required, rotate MFA and application secrets together with [secret-master-key-rotation.md](secret-master-key-rotation.md); changing the environment variable alone makes existing encrypted state unreadable. Deploy matching API and web assets after the `todo.md` package checks. The `.28` development host's password-only policy was deployed and verified on 2026-09-20; see [the live acceptance record](history/owner-mfa-optional-webrich-live-acceptance-2026-09-20.md).
