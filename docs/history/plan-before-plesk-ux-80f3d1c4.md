...
## Açık işler — 2026-09-22 YunPanel site yaşam döngüsü ve arayüz (Antigravity)

**Durum: açık.** Aşağıdaki her `[ ]` ayrı kapanış ölçütüdür; ilk kaynak dilimleri bulunması işin veya canlı kabulün tamamlandığı anlamına gelmez. Canlı `/dashboard`, `/settings`, `/databases` ve `/websites/new` sekmelerinde görülen sorunlar bu maddelerin girdisidir. Kaynak işleri burada, `.28` host/tarayıcı kabulü `todo.md` içindeki T-DB-UI ve T-VISUAL'dadır. `.44` kapsam dışı; `agents.md` ve `docs/architecture.md` sınırları korunur. Antigravity mevcut durable job, auth ve reusable React bileşenlerini kullanır; TypeScript, browser `prompt`/`alert`, ikinci Roundcube instance veya ikinci root yönetim yolu eklemez.
...
- [ ] YP-13–YP-16 arayüz işleri kaynak ve T-VISUAL kabulünden sonra kapatılsın; tasarımın başlaması diğer backend maddelerinin bitmesini beklemez.
