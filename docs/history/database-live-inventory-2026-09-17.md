...
- Socket/provider hatası, system schema, duplicate veya malformed inventory raw host ayrıntısı taşımadan `database_inventory_unavailable` olur.
- React Database ekranı açılışta ve Yenile eyleminde canlı GET kullanır; “Sunucuyu tara” job'u günlük yüzeyden çıkarılmıştır. Create/drop yine typed confirmation'lı durable job olarak kalır.
...
Odak API/web ve production wiring testleri 25/25 geçti. Gerçek local socket GET/refresh kabulü `todo.md` T-DATABASE altında kalır.
