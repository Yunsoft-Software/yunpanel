...
- `MAIL_CONFIG_ROLLBACK` protocol operation'ı strict ve secret-free bir mutation envelope'u olarak tanımlıdır. Payload source apply job, mail domain, previous/current revision-status matematiği ile current configuration/source plan/backup/preview digest'lerine bağlanır; raw config, backup path'i ve genişletilmiş alanlar reddedilir. Executor hazırdır fakat restart resolver tamamlanmadan kontrat HTTP enqueue yüzeyine açılmamıştır.
