...
### 18 Eylül continuation — exact PHP container control-plane metadata migration

- `a5347c53`, `2a3a3924`, `19670e19` ve `1a198b7b` PHP isolation migration ile source provisioning/release identity'sini ayırdı. Durable journal original release operation ID'sini `sourceOperationId` olarak pinler; yeni isolation migration ID yalnız migration receipt lifecycle'ını sahiplenir. Böylece PHP container current release lookup'u migration UUID'sini yanlışlıkla release ID gibi kullanmaz.
- `87ff8bbe` + `cf71da1a` PHP container manager'a normal provisioning `apply()` yolundan ayrı receipt-backed migration lifecycle ekledi. Receipt mutation öncesi exact Application/Website/source release/path spec digest'i ile `applicationRoot`, `releasesDirectory` ve `current` symlink previous UID/GID/mode metadata'sını root-private state'e yazar. Safe candidate yalnız release directory + public root site UID/GID `0750` kaldığında ve current symlink exact source release'i gösterdiğinde açılır.
- Migration apply/rollback yalnız bu üç control-plane path'ine non-recursive `chown/chmod` uygular. Release content'e recursive ownership/mode mutation veya delete yoktur. `20e81722` + `4c29f803` apply/rollback öncesi directory/symlink path type'larını tekrar doğrulayıp TOCTOU symlink replacement'ını fail-closed yaptı.
- `742cce93`, `2e68afb2`, `e17d8cb8` ve `cabf459e` composite PHP preview/audit authority'sini daralttı. `repair_php_container_metadata` ancak container exact safe candidate, canlı site FPM runtime sağlıklı, package/pool/config/service/socket evidence canonical ve shared `UMask=0027` iken uygulanabilir. Bounded audit safety flag'lerini kendisi tekrar türetir; çelişkili `fpmRuntime=true` evidence apply authority açmaz.
- `d0e32af7`, `5cc42bbd` ve `67d6a523` durable isolation journal/runtime'ına FPM pool `adapter=php` operation'ından ayrı `adapter=php_container` lifecycle ekledi. Restart completed receipt'i inspect ederek kapatır, incomplete mutation'ı kör replay etmez; typed rollback yalnız container receipt evidence'ına gider.
- `8bfa9d90` rollback recovery'yi receipt-finalization'a bağladı: host metadata previous state'e dönmüş olsa bile receipt state `compensated` değilse recovery operation'ı tamamlanmış saymaz. Böylece prepared/active receipt registry tarafından sessizce kapanmaz.
- `1b91c7aa` + `35e42aa4` Website isolation panelinde PHP container metadata migration'ını PHP-FPM pool migration'ından ayrı sunar; apply/rollback copy'si exact three-path scope'u ve recursive mutation yapılmadığını açıklar.
- Source-level test kontratları host receipt apply/restart/rollback, foreign metadata drift, path-type drift, source-release/migration-ID ayrımı, audit evidence consistency, durable journal/runtime recovery ve UI presentation için eklendi/güncellendi. Bu oturumda supported Node runtime/gerçek Ubuntu acceptance çalıştırılmış sayılmadı; gerçek host acceptance `todo.md` altında kalır.
- P0.1 source tarafında workspace, canonical identity, SFTP, missing-only PHP-FPM pool ve exact PHP control-plane container metadata repair operation-owned lifecycle'a sahiptir. Passenger legacy runtime ve static legacy permissions hâlâ explicit blocked-authority state'tedir; geniş PHP release/content/path drift'i de fail-closed kalır.


### 18 Eylül continuation — Passenger isolation handoff to canonical cutover workflow

- Repository audit showed legacy direct-systemd → Passenger migration already has a dedicated authority path outside the isolation journal: read-only digest-bound preview, typed confirmation, async migration job, job UUID-bound host operation, Passenger environment ownership receipt, source/target Nginx checksum verification, health-gated cutover, safe rollback on target health failure, legacy systemd cleanup with `cleanup_required`, queued Website/Domain revision authority snapshot and `ApplicationRuntimeBinding` reconciliation.
- Because successful Passenger cutover changes the runtime authority rather than creating a narrow reversible filesystem artifact, isolation journal was intentionally **not** extended with a fake Passenger rollback adapter. `passenger-site-manager` keeps `automaticMigration=false` / `passenger_legacy_runtime_not_operation_owned` so generic isolation apply remains fail-closed.
- `18a23ac9`, `8e5ed103`, `088ad94c`, `49acdbb5` and `45d5ad06` connect Website isolation UI to the canonical Application Passenger preview/apply workflow. The panel loads exact blockers/preview, requires the workflow's own `migrate-node-passenger:<applicationId>:<digest>` confirmation, queues the durable Passenger job, exposes the job ID and refreshes workspace state. Generic isolation copy now explains that Passenger runtime drift is managed by the dedicated workflow instead of claiming that all migration capability is unavailable.
- Real Owner-session UI handoff, queued/running routing serialization, target health, restart/reconciliation and `cleanup_required` acceptance remain in `todo.md`. Source P0.1 is now narrowed to static legacy publish/release permission + ACL ownership/rollback authority; Passenger isolation mutation stays intentionally outside the isolation journal.


### 18 Eylül continuation — static control-plane metadata durable migration

- `37734479` + `71e7ed6c` static publish manager'a normal provisioning `apply()` yolundan ayrı root-private migration receipt ve lifecycle ekledi. Safe candidate yalnız canonical identity + mevcut ACL tooling + en az bir managed release + bütün release dosya/dizinlerinin site UID/GID ve `0750/0640` mode + Nginx read ACL açısından zaten sağlıklı olduğu durumda, farklar sadece `publishRoot`/`releasesRoot` control metadata veya `current` symlink owner drift'i ise açılır.
- Migration receipt host mutation'dan önce previous `publishRoot` ve `releasesRoot` UID/GID/mode, `current` UID/GID ve exact current target'ı yazar. Apply yalnız bu üç exact path'i non-recursive düzeltir; migration yolunda `setfacl -R`, release-tree chown/chmod veya recursive remove yoktur. Restart `inspectMigrationOperation` ile completed receipt'i kapatır.
- Rollback current target ve release tree/ACL safety gate'ini tekrar doğrular; yalnız desired/previous metadata state'inden previous receipt metadata'sına döner. Foreign UID/GID/mode, path-type/symlink replacement veya current-target drift destructive rollback'i bloklar. Receipt state `compensated` finalize edilmeden wrapper/recovery compensation tamamlanmış saymaz.
- `3a0ab184` + `008b9331` dedicated static control lifecycle'ını API wrapper'a çıkardı; control migration normal static deploy `apply()`/release compensation yolunu çağırmaz.
- `bf828c97`, `6bb67d63`, `b0cb1050` ve `20ce451f` isolation audit, typed `adapter=static_control` journal, restart-safe executor ve production capability gate zincirini ekledi. Audit `safeMigrationCandidate` ve `safeControlMigrationCandidate` değerlerini bounded evidence üzerinden tekrar türetir; ACL/release drift'i bu adapter üzerinden apply açamaz.
- `2eab2488`, `5b9f7a26`, `25b421cd`, `8bb647af`, `b90ca6ed`, `a794ac23`, `d3c86502` ve takip test commitleri manager/wrapper/audit/journal/runtime/bootstrap source kontratlarını kapsar. `f31847c8` + `bf027d9c` panelde static control operation'ını ayrı sunar ve release dosyaları/ACL'lerin migration kapsamı dışında olduğunu açıklar.
- Bu checkpoint'te P0.1 source'ta static control metadata authority tamamlandı. Kalan static source işi managed release file/directory ownership-mode ve Nginx read ACL drift'i için bounded operation-owned snapshot/apply/exact rollback lifecycle'ıdır. Gerçek Ubuntu static control acceptance `todo.md` içinde kalır; bu oturumda real-host acceptance çalıştırılmış sayılmaz.
