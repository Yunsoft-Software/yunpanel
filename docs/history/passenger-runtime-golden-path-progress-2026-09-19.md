...
Bu çalışma, `plan.md` altındaki `P1.1 — Runtime golden path` bölümünün şu maddesini ilerletir; madde henüz tamamlanmış değildir:
...
- **`passenger-site-manager.js`**:
  ...
  - **Setup Validation Helper (`validatePassengerSetup`)**:
    - Node major, startup file uzantısı, absolute app root ve canonical `yunapp-*` kimliğinin salt okunur girdi doğrulamasını yapan yardımcı eklendi. Bu helper hosttaki Nginx/Passenger paketini veya Node binary'sini denetlemez; bunlar mevcut Passenger inspector/manager ve gerçek Ubuntu kabul kapılarının sorumluluğudur.
...
- **`passenger-runtime-golden-path.test.js`**:
  - **Setup Input Validation**: Desteklenmeyen Node major, startup uzantısı ve Unix kimliğinin doğru hata kodlarıyla reddedildiği doğrulandı. Gerçek Nginx Passenger modülü ve Node runtime dependency kabulü bu testin kapsamında değildir.
...
- Passenger dependency/package health, environment activation, startup/config validation ve rollback zincirinin gerçek Ubuntu paketinde uçtan uca kabulü tamamlanmadı.
- `plan.md` P1.1 Passenger maddesi bu nedenle açık kalır; gerçek-host kanıtı olmadan bu kaynak dilimi production-ready sayılmaz.
