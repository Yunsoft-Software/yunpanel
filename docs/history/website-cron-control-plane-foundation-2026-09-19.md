...
- Registry ile canlı `/etc/cron.d` state'ini birlikte authoritative inventory yapan reconciliation provider henüz yok.
...
Bu sohbet ortamında Node 24 checkout/runner olmadığı için source testleri çalıştırılmadı. Hedefli test/syntax ve gerçek Ubuntu `cron`/`/etc/cron.d` ownership/service davranışı `todo.md` içinde açık tutulur.
