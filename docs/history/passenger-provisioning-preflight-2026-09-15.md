...
- Headed Owner login and the server terminal were exercised through the public same-origin gateway. The panel Nginx vhost was missing WebSocket forwarding headers; its previous file was saved in the deployment rollback directory, the headers were added behind an `nginx -t` gate, and Nginx was reloaded successfully.
