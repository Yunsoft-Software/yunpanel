...
Passenger legacy runtime ve static runtime preview'ları `automaticMigration=false` kalır. Passenger state isolation journal içinden mutate edilmez; Node Website paneli canonical Application Passenger workflow'una geçer. Static tarafta yalnız `static_control` three-path metadata repair authority'si açıktır; managed release file/directory ownership-mode ve Nginx ACL drift'i receipt-backed content authority oluşana kadar blocked kalır. PHP'de de yalnız exact three-path control-plane metadata repair authority'si açıktır; release content ownership/mode, current target, path topology veya shared runtime drift'i bunun dışındadır ve fail-closed kalır.
