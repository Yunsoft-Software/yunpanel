...
Passenger legacy runtime ve static legacy permissions preview'ları explicit blocked-authority state taşır (`automaticMigration=false`). Passenger state isolation journal içinden mutate edilmez; Node Website için panel canonical Application Passenger migration preview/apply workflow'una geçer. O ayrı workflow durable async job UUID'sini operation ownership olarak kullanır, queued Website/Domain revision authority snapshot'ını pinler, source ve target Nginx checksum'larını doğrular, target health geçmeden legacy systemd cleanup yapmaz ve terminal host sonucunu `ApplicationRuntimeBinding` reconciliation ile authority'ye çevirir. Health/config cutover başarısızlığında eski route ancak source hâlâ sağlıklı ve rollback checksum'u kanıtlıysa geri alınır; başarılı migration sonrası generic isolation rollback eski systemd'yi yeniden etkinleştirmez. Static legacy permissions/ACL ise receipt-backed authority oluşana kadar blocked kalır. PHP'de yalnız yukarıdaki exact three-path control-plane metadata repair authority'si açıktır; release content ownership/mode, current target, path topology veya shared runtime drift'i bunun dışındadır ve fail-closed kalır.
