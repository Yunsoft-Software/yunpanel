...
Every public certificate inventory/detail/import/selection/issue response uses the same bounded view. It omits ACME email, PEM, all certificate material paths, material digests and raw stored failure codes. It exposes only safe certificate metadata, bounded challenge/provider/credential/zone identity and an authored lifecycle diagnosis, including in-progress, validated, superseded, expiring, expired and catalogued failure states. Completed public job results also omit certificate, full-chain and private-key paths even though the private durable result retains exact paths for reconciliation and recovery.

Common management audit records only the bounded Domain-scoped action and outcome; request bodies and job results are not audit metadata. Certificate registry state upgrades in memory from version 1 ACME records through version 2 source/renewal metadata to version 3 `certificateNames`/challenge metadata and is written only on the next mutation.
