...
This section records the limitations of the dated increment above; its former `plan.md` A, T-USER and T-UI-RUNTIME handoff labels no longer exist. Current authentication/browser work is tracked under `todo.md` T-AUTH-AUDIT, while package and end-to-end browser gates remain under T-PACKAGE-LIVE and T-FEATURE-ACCEPTANCE. Do not label the whole panel production-ready on the basis of these historical model tests.
