...
- [x] Shared site mutation lock: `2fa0a7bf` → `bbf4774f`; create/provisioning/retry/compensation/removal ile PHP queue+worker ve cron worker aynı Application/Website lock root'unda seri hale gelir. `2d2ee234`, `74a410e1`, `4019496d`, `368e5f3e`, `1e56e738`, `ed67123c`, `f2da5ecd` kaynak testlerini ekler; gerçek iki süreç/crash/permission kabulü açık.
- [x] Hosting quota release: `71f738bb` → `8267d036`; allocation yalnız verified Website/Application absence sonrası removal final step'te transaction içinde bırakılır. Timeout/yarım cleanup/reply kaybı release değildir; release failure `removed` durumunu engeller ve explicit continue ile idempotent tekrar edilir. `ad000a36` / `a8f4f613` kaynak testleri eklendi.
- [x] Process-shared journal source: `d67410f4` → `2bb7960e`, `f8db6dd5`, `c7c732a1`, `12de5caa`, `f4778c4f`; job/recovery/removal/provisioning JSON store'ları store-wide lock + reload modeline taşındı. `6e43ab23`, `c6d43863`, `99c03ed6`, `f3d20f25` cross-instance kaynak testleri eklendi; gerçek OS-process kabulü açık.
- [x] Cron/removal live auth: `d0704aa4` → `49512a45`; user cron jobs actor session/user/role pinler ve local worker host mutation öncesi live session/MFA/Website grant recheck yapar. Removal child cron `system_removal` payload'ını deterministic tutar fakat `d954417d` / `9a2cc2c6` ile worker aktif removal journal + planned task + private Owner actor'u çözüp auth store'dan live Owner/MFA recheck yapmadan host remove başlatmaz. `51375637`, `b5263cf5`, `e765441f`, `f49b1d7b`, `5508f518` test kaynağı.
- [x] Website removal Owner journal auth: `d97c4a51` → `4c4d5b62`; start/continue exact Owner identity ile bağlanır, session revoke/disable/MFA kaybı destructive continuation'ı durdurur, farklı Owner devralamaz. Aynı Owner yeni login session'ıyla devam ettiğinde private journal session evidence rotate edilir; actor public operation JSON'a girmez.
...
- [x] Kaynak: `d67410f4` → `2bb7960e` + `f8db6dd5` / `c7c732a1`; durable JobRegistry ve recovery sidecar process-store lock altında reload-before-read/write kullanır. `6e43ab23` / `c6d43863` bağımsız registry instance'larında lost-update ve terminal reconciliation görünürlüğünü kapsar. [ ] Gerçek iki OS API/worker process, lock crash/permission ve Node24/npm11 kabulü ayrıca çalıştırılmalı.
...
- [ ] Güvenli cleanup/release kabulü: Website removal finalizasyonuna verified allocation release kaynakta bağlandı; timeout/başarısız cevap otomatik rezervasyon silmez. Kalan: provisioning compensation ile hiç-Website-oluşmamış reservation recovery, gerçek restart/write-failure/two-process kabulü ve doğru müşteri/site/iş audit kanıtı. Doğrudan SQL silme kabul değildir.
