...
- [ ] **PHP-ACTION-02c HTTP/UI ve canlı yetki:** reviewed queue service kaynakta var fakat production HTTP execution route'una bağlanmadı ve UI butonu açılmadı. Site_manager'ın enqueue sonrası yetkisi/Website sahipliği işlem başlayana kadar canlı yeniden doğrulanmadan ve aynı Application için süreçler arası enqueue kilidi/yarış kabulü tamamlanmadan kullanıcı akışı açılmayacak. Eski Owner raw endpoint'i Plesk-benzeri güvenli kullanıcı akışı sayılmaz.
