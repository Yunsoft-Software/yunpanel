...
- `012e68bf` → `2eccf992`: hosted create reserve→attach lock boşluğu kapandı; hiç Website/planned Domain/Mail/operation-owned Application oluşmamış reserved hold explicit recovery ile bırakılabilir. Provisioning journal mevcutsa recovery fail-closed kalır.
...
- Provisioning child host job worker-anı live auth, provisioning journal compensation terminalization, gerçek iki OS process/lock crash-permission, write-failure/cold-restart ve Node24/npm11/host/browser kabulü hâlâ açıktır.
