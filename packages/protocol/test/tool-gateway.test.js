...
test('integrated tool gateway descriptors keep unique audience, prefix, access path and Unix socket', () => {
  ...
  assert.deepEqual(values.map((value) => value.id), ['phpmyadmin', 'elfinder', 'ttyd', 'netdata', 'goaccess']);
  ...
  assert.deepEqual(integratedToolGateway('goaccess'), {
    id: 'goaccess',
    audience: 'goaccess',
...
test('management access path lookup never broad-matches tool prefixes', () => {
  ...
  assert.equal(isManagementToolGatewayAccessPath('/api/goaccess-gateway-access'), true);
  ...
  assert.equal(managementToolGatewayForAccessPath('/api/goaccess-gateway-access')?.id, 'goaccess');
