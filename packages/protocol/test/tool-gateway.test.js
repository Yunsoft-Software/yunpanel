...
test('integrated tool gateway descriptors keep unique audience, prefix, access path and Unix socket', () => {
  ...
  assert.deepEqual(values.map((value) => value.id), ['phpmyadmin', 'elfinder']);
  for (const field of ['audience', 'publicPrefix', 'accessPath', 'socketPath']) {
...
test('descriptor validation rejects expanded or unsafe gateway shapes', () => {
  ...
  for (const invalid of [
    {},
    { id: 'x', audience: 'x', publicPrefix: '/tools/x/', accessPath: '/api/x', socketPath: '/run/x.sock' },
    { id: 'x', audience: 'x', publicPrefix: '../x', accessPath: '/api/x', socketPath: '/run/x.sock' },
    { id: 'x', audience: 'x', publicPrefix: '/tools/x', accessPath: '/api/x?bad=1', socketPath: '/run/x.sock' },
    { id: 'x', audience: 'x', publicPrefix: '/tools/x', accessPath: '/api/x', socketPath: '/tmp/x' },
  ]) assert.throws(() => descriptor(invalid), /descriptor is invalid/);
  assert.throws(() => integratedToolGateway('ttyd'), /Unknown integrated tool gateway/);
});
