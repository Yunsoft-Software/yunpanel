...
const UNIT_PATTERN = /^yun(?:panel|agent)-[a-z0-9@_.-]+\.service$/;
const DEFAULT_RESTART_UNITS = Object.freeze([
...
function normalizeRestartUnits(value) {
...
export function createSystemPackageManager({
  ...
  restartUnits = DEFAULT_RESTART_UNITS,
  now = Date.now,
} = {}) {
  const serviceUnits = normalizeRestartUnits(restartUnits);
...
export const systemPackageManagerInternals = Object.freeze({
  normalizeVersion,
  parsePolicy,
  packageState,
  normalizeRestartUnits,
  defaultRestartUnits: DEFAULT_RESTART_UNITS,
});
