...
export async function inspectHostInventory({ mode = 'local', cpuSampleMs = 100 } = {}) {
  const cpus = os.cpus();
  const totalMemory = os.totalmem();
  const freeMemory = os.freemem();
  const [operatingSystem, cpuUsagePercent, filesystem, capabilities] = await Promise.all([
    readOperatingSystem(), sampleCpuUsage(cpuSampleMs), inspectRootFilesystem(), detectCapabilities(),
  ]);
