...
export function createDockerInspector({ accessFn = access, execFn = execFileText } = {}) {
  return async function inspectDocker() {
    ...
    if (!dockerPath) {
      return { installed: false, reachable: false, path: null, version: null, composeVersion: null, containers: [], errorCode: null };
    }
    try {
      ...
      return { installed: true, reachable: false, path: dockerPath, version: null, composeVersion: null, containers: [], errorCode: safeFailureCode(error) };
    }
  };
}
