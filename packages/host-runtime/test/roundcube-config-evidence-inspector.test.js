...
function preview() {
  return {
    ...
    nginx: {
      artifact: { path: roundcubeNginxTemplatePolicy.configPath, sha256: sha256(NGINX) },
...
function fixture({ databaseMode = 0o600, groups = 'yunpanel-roundcube www-data', failHttp = false } = {}) {
  const calls = [];
  const inspector = createRoundcubeConfigEvidenceInspector({
    run: async (command, args, options = {}) => {
      ...
      if (command === '/usr/bin/curl' && failHttp) throw new Error('unhealthy endpoint');
      return { stdout: '' };
    },
    lstatFn: async (target) => {
      ...
      if (target === roundcubeNginxTemplatePolicy.configPath) return file({ uid: 0, gid: 0, mode: 0o640, size: NGINX.length });
    ...
    readFileFn: async (target) => {
      if (target === roundcubeTemplatePolicy.configPath) return CONFIG;
      if (target === roundcubeNginxTemplatePolicy.configPath) return NGINX;
      return FPM;
    },
  });
...
test('live Roundcube evidence proves exact config, FPM, Nginx and HTTPS final state', async () => {
  const fx = fixture();
  ...
  assert.ok(fx.calls.some(([command, args]) => command === '/usr/sbin/nginx' && args[0] === '-t'));
  assert.ok(fx.calls.some(([command, args]) => command === '/usr/bin/curl' && args.includes('https://mail.example.com/')));
});

test('live Roundcube evidence fails closed on unsafe host state or unhealthy HTTPS endpoint', async () => {
  ...
  assert.equal((await fixture({ failHttp: true }).inspector.inspect(preview())).satisfied, false);
});