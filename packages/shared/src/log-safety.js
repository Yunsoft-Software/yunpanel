...
const SENSITIVE_ASSIGNMENT = /\b([A-Za-z0-9_.-]*(?:password|passwd|pwd|secret|token|api[-_]?key|private[-_]?key|access[-_]?key|cookie)[A-Za-z0-9_.-]*\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^\s,;]+)/gi;
...
export function sanitizeLogMessage(value) {
  ...
  const redacted = bounded
    ...
    .replace(AUTHORIZATION_VALUE, (match, prefix, scheme) => `${prefix}${scheme ? `${scheme} ` : ''}[REDACTED]`)
