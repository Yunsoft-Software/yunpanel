...
- Fresh local-mail Website provisioning reserves a disabled Mail Domain with no default mailbox/password, then source-plans operation-owned `mail_config → mail_dkim_key → mail_dns_reapply → mail_dkim_config → roundcube_mapping → webmail_dns_reapply` steps after the main Website certificate/TLS activation. DKIM key identity, authoritative desired-state publication, signing-config child jobs and shared Roundcube apply are revision/digest/operation fenced and recoverable without blind mutation replay.
- Shared Roundcube source lifecycle uses one application/FPM/database with revisioned per-domain `webmail.<domain>` mappings, certificate coverage fences, Nginx desired state, DNS readiness gating and Domain-removal cleanup semantics. Fresh Website provisioning now binds mapping/apply plus post-apply webmail DNS re-apply; the remaining source gap is an independent operation-owned `purpose: webmail` certificate lifecycle, followed by live webmail acceptance.
