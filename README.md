...
- Site file workers and site terminals run through fixed arguments as the deterministic `yunapp-*` account. Website cron records are likewise pinned to the exact managed `yunapp-*` identity; the host primitive renders only deterministic root-owned `/etc/cron.d` files and refuses foreign ownership/drift, while public cron mutation stays closed until durable execution/reconciliation is complete.
...
- Shared Roundcube source lifecycle uses one application/FPM/database with revisioned per-domain `webmail.<domain>` mappings, dedicated webmail-certificate coverage fences, Nginx desired state, DNS readiness gating and Domain-removal cleanup semantics. Fresh Website provisioning now issues/reconciles an independent operation-owned `purpose: webmail` certificate after authoritative DNS readiness and before mapping/apply, serves autodiscover/autoconfig from exact TLS-only Website paths through a managed Unix socket, then blocks readiness on current mail-service/listener/Roundcube/discovery endpoint health; live Ubuntu/Certbot/Roundcube mailbox-auth/browser/mail-client and delivery acceptance remain open.
...
- Owner-only Website/Domain move-delete impact previews that enumerate current hierarchy, Application/Docker binding, DNS-zone, mail-domain, mailbox, certificate and active-job dependencies. Backup inventory is now evidence-backed; cron remains fail-closed even though its registry/template/host primitive foundation exists, because authoritative live reconciliation is not yet wired. No cascade or public full-delete apply route is implied.
