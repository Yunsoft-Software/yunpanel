...
- Operation-specific running recovery for read-only package/service/database/Node status inspection, domain stage/activate, static deploy/rollback, Node deploy/rollback/restart/process control, database create/delete, managed-service mutations, YunPanel package upgrade and certificate issue/renew. Recovery uses exact persisted intent plus host evidence or private job-bound receipts; there is no generic force-success or blind mutation retry.
