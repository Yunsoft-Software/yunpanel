...
async function findSystemctl() {
  for (const candidate of SYSTEMCTL_PATHS) {
    try {
      ...
      // Continue through the fixed systemctl path allowlist.
    }
  }
  return null;
}
...
async function inspectUnit(systemctlPath, unit) {
  ...
  try {
    const { stdout } = await execFileAsync(systemctlPath, args, {
      encoding: 'utf8',
      timeout: 2500,
      maxBuffer: 64 * 1024,
      windowsHide: true,
...
export const systemdInspectionPolicy = Object.freeze({
