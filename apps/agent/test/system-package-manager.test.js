...
} from '../src/system-package-manager.js';
...
test('upgrade refreshes APT, upgrades only YunPanel and schedules delayed restarts', async () => {
  const calls = [];
  ...
  const manager = createSystemPackageManager({
    ...
    run: async (file, args) => {
      ...
      if (file === '/usr/bin/apt-cache') {
        return { stdout: `Installed: ${installedVersion}\nCandidate: 0.2.0-1\n` };
      }
