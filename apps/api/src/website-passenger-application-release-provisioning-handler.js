...
export class WebsitePassengerApplicationReleaseProvisioningError extends Error {
  constructor(code, message, status = 409) {
    super(message);
    this.name = 'WebsitePassengerApplicationReleaseProvisioningError';
    this.code = code;
    this.status = status;
  }
}
...
function activationContext(context = {}) {
  ...
  if (!operation || typeof operation !== 'object' || operation.operationId !== operationId
    ...
    || intent.adapter !== 'passenger-application-release'
    || typeof intent.applicationId !== 'string'
    || intent.releaseId !== operationId) {
    throw new WebsitePassengerApplicationReleaseProvisioningError(
      'website_passenger_application_release_intent_invalid',
      'Passenger Application release intent does not match the provisioning operation',
      400,
    );
  }
  const planned = operation.resources?.application;
  const release = stepEvidence(operation, 'node_release');
  if (!planned || planned.id !== intent.applicationId || planned.type !== 'node'
    || planned.runtimeAdapter !== 'passenger' || !planned.runtime
    || !release || release.adapter !== 'passenger-release'
    ...
    throw new WebsitePassengerApplicationReleaseProvisioningError(
      'website_passenger_application_release_evidence_invalid',
      'Passenger Application release evidence is incomplete or drifted',
    );
  }
...
export function createWebsitePassengerApplicationReleaseProvisioningHandler({ applicationRegistry } = {}) {
  if (!applicationRegistry
    || typeof applicationRegistry.getApplication !== 'function'
    || typeof applicationRegistry.activatePassengerRelease !== 'function'
    || typeof applicationRegistry.resetPassengerInitialRelease !== 'function') {
    throw new WebsitePassengerApplicationReleaseProvisioningError(
      'website_passenger_application_release_dependencies_invalid',
      'Passenger Application release provisioning dependencies are invalid',
      503,
    );
  }
  ...
  async function inspect(context = {}) {
    ...
    if (!application) {
      return Object.freeze({ satisfied: false, reason: 'website_passenger_application_missing' });
    }
    if (application.type !== 'node' || application.runtimeAdapter !== 'passenger') {
      throw new WebsitePassengerApplicationReleaseProvisioningError(
        'website_passenger_application_release_adapter_drift',
        'Application runtime adapter changed before Passenger release reconciliation',
      );
    }
    if (application.currentReleaseId !== spec.releaseId) {
      return Object.freeze({
        satisfied: false,
        reason: 'website_passenger_application_release_pending',
        applicationId: spec.applicationId,
        releaseId: spec.releaseId,
        currentReleaseId: application.currentReleaseId,
      });
    }
    if (application.lastDeploymentId !== spec.operationId
      || application.previousReleaseId !== spec.previousReleaseId
      || application.currentCommitSha !== spec.commitSha
      || JSON.stringify(application.activeRuntime) !== JSON.stringify(spec.runtime)
      ...
      throw new WebsitePassengerApplicationReleaseProvisioningError(
        'website_passenger_application_release_state_drift',
        'Application state conflicts with Passenger release evidence',
      );
    }
  ...
  async function apply(context = {}) {
    ...
    const application = await applicationRegistry.activatePassengerRelease(spec.applicationId, {
  ...
  async function inspectCompensation(context = {}) {
    ...
    if (spec.previousReleaseId !== null) {
      return Object.freeze({
        satisfied: false,
        reason: 'website_passenger_application_release_compensation_requires_previous_release_restore',
      });
    }
    const application = await applicationRegistry.getApplication(spec.applicationId);
    ...
    if (application.currentReleaseId !== spec.releaseId) {
      throw new WebsitePassengerApplicationReleaseProvisioningError(
        'website_passenger_application_release_compensation_drift',
        'Application release changed before Passenger release compensation',
      );
    }
    return Object.freeze({ satisfied: false, reason: 'website_passenger_application_release_compensation_pending' });
  }
  ...
  async function compensate(context = {}) {
    ...
    await applicationRegistry.resetPassengerInitialRelease(spec.applicationId, {
      operationId: spec.operationId,
      releaseId: spec.releaseId,
    });
...
export const websitePassengerApplicationReleaseProvisioningInternals = Object.freeze({
  stepEvidence,
  activationContext,
  publicEvidence,
});
