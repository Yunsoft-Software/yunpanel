...
function validateWebsite(website, localServerId) {
  ...
  if (!['static', 'node', 'php', 'python'].includes(website.runtimeType) || !UUID_PATTERN.test(website.applicationId ?? '')) {
  ...
  const current = website.documentRoot || (website.runtimeType === 'static' ? `/var/www/yunpanel/apps/${applicationId}/current` : `/var/lib/yunpanel/apps/${applicationId}/current`);
  const user = website.unixUser || appUnixUser(applicationId);
  if (!APP_USER_PATTERN.test(user)) {
...
async function resolveRelease(target, dependencies) {
  ...
  if (!info.isDirectory()) {
    throw new SiteFileManagerError('site_files_release_invalid', 'Website active root is not a directory', 409);
  }
  return root;
}
