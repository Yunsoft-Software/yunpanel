...
import { bindLegacyDomainToWebsite, WebsiteMigrationBindError } from './website-migration-bind.js';
import { previewWebsiteMigration } from './website-migration-preview.js';

const BIND_FIELDS = new Set(['domainId', 'websiteId', 'confirmation']);
...
function assertBindBody(body) {
  ...
  if (body.confirmation !== `bind:${domainId}:${websiteId}`) {
    throw new WebsiteMigrationBindError('website_migration_confirmation_required', `Confirm migration binding with bind:${domainId}:${websiteId}`, 400);
  }
  return Object.freeze({ domainId, websiteId });
}
