...
const DEFAULT_ROOT = '/var/lib/yunpanel/recovery/node-rollbacks';
...
const RECEIPT_KEYS = new Set([
  ...
  'serviceName', 'port', 'healthPath',
]);

export class NodeRollbackReceiptError extends Error {
  constructor(code, message) {
    super(message);
    this.name = 'NodeRollbackReceiptError';
    this.code = code;
  }
}
...
function uuid(value, label) {
  if (typeof value !== 'string' || !ID_PATTERN.test(value)) {
    throw new NodeRollbackReceiptError('node_rollback_receipt_identity_invalid', `${label} identity is invalid`);
  }
...
function normalize(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    ...
    throw new NodeRollbackReceiptError('node_rollback_receipt_invalid', 'Node rollback receipt metadata is invalid');
  }
  return Object.freeze({
    ...
    previousReleaseId: uuid(value.previousReleaseId, 'previous release'),
...
export function createNodeRollbackReceiptStore({ root = DEFAULT_ROOT, now = () => Date.now() } = {}) {
  if (typeof root !== 'string' || !path.isAbsolute(root) || typeof now !== 'function') {
    throw new NodeRollbackReceiptError('node_rollback_receipt_dependencies_invalid', 'Node rollback receipt store configuration is invalid');
  }

  function receiptPath(serverId, jobId) {
    if (typeof serverId !== 'string' || !SERVER_ID_PATTERN.test(serverId)) {
      throw new NodeRollbackReceiptError('node_rollback_receipt_identity_invalid', 'Server identity is invalid');
    }
    return path.join(root, serverId, `${uuid(jobId, 'job')}.json`);
  }

  async function write({ serverId, jobId, applicationId, result }) {
    if (!result || result.healthy !== true || result.active !== true) {
      throw new NodeRollbackReceiptError('node_rollback_receipt_result_invalid', 'Node rollback result is not safe recovery evidence');
    }
  ...
  async function read(serverId, jobId) {
    ...
    try {
      ...
      throw new NodeRollbackReceiptError('node_rollback_receipt_read_failed', 'Node rollback receipt could not be read');
    }
...
export const nodeRollbackReceiptInternals = Object.freeze({ storeVersion: STORE_VERSION, defaultRoot: DEFAULT_ROOT, normalize });
