...
test('production API shares one managed DKIM desired-state service with HTTP and local execution', async () => {
  ...
  assert.match(indexSource, /createApp\(\{[\s\S]*?mailDkimRegistry,[\s\S]*?mailDiagnosticsInspector,[\s\S]*?mailDkimConfigurationService,[\s\S]*?\}\)/);
  ...
  assert.match(appSource, /mountMailDkimRoutes\(app,\s*\{[\s\S]*?mailDkimRegistry,[\s\S]*?mailDkimConfigurationService,[\s\S]*?jobRegistry:\s*mailDkimConfigurationService\s*\?\s*jobRegistry\s*:\s*null,[\s\S]*?\}\)/);
...
test('production package and lost-ack recovery use the canonical DKIM root and recovery command', async () => {
