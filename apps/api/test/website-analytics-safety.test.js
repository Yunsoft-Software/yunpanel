...
const tool={satisfied:true,binaryPath:'/usr/bin/goaccess',version:'1.8.1'};

test('analytics status hides pid and host socket paths',()=>{
 const value=websiteAnalyticsHttpInternals.analyticsStatusView(raw,tool,websiteId,{owner:false});
 assert.deepEqual(value,{websiteId,available:true,version:'1.8.1',running:true,socketReady:true});
...
test('owner status gets only same-origin websocket url, not host path',()=>{
 const value=websiteAnalyticsHttpInternals.analyticsStatusView(raw,tool,websiteId,{owner:true});
...
test('analytics status rejects malformed tool evidence',()=>{assert.throws(()=>websiteAnalyticsHttpInternals.analyticsStatusView(raw,{satisfied:'yes'},websiteId));});
