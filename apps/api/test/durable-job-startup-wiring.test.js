...
test('production API shares one durable job registry across HTTP, scheduler and local execution', async () => {
  ...
  assert.match(source, /const jobRegistry = createDurableJobRegistry\(\{[\s\S]*?filePath: jobStorePath,[\s\S]*?registryFactory: createJobRegistry,[\s\S]*?automaticReconciliation: true,[\s\S]*?\}\);/);
