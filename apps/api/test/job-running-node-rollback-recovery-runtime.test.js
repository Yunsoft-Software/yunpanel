...
import { runRunningNodeRollbackRecoveryFromStores } from '../src/job-running-node-rollback-recovery-runtime.js';

const serverId = 'server-1';
const jobId = '7f217caa-0f0f-4569-a657-30a97bcb7ca0';
const hostname = 'host-1';

test('Node rollback runtime binds exact host, application state, private receipt and read-only status', async () => {
  const calls = [];
  const fakeJobRegistry = { marker: 'durable' };
  const fakeApplicationRegistry = { async init() { calls.push(['application.init']); } };
  const result = await runRunningNodeRollbackRecoveryFromStores({
    serverId,
    jobId,
    hostname,
    env: {
    ...
    statusInspectorFactory: () => ({
      async inspectNodeStatus(intent) {
        ...
        return { releaseId: 'release-1', healthy: true };
      },
    }),
    ...
    recoverCommand: async (input) => {
      ...
      assert.equal((await input.readRollbackReceipt(serverId, jobId)).jobId, jobId);
      assert.deepEqual(await input.inspectNodeStatus({ applicationId: 'app' }), { releaseId: 'release-1', healthy: true });
      return {
        serverId,
        jobId,
...
test('wrong host stops Node rollback recovery before application, receipt or status construction', async () => {
  let applicationFactories = 0;
  let receiptFactories = 0;
  let statusFactories = 0;
  await assert.rejects(
    runRunningNodeRollbackRecoveryFromStores({
      serverId,
      jobId,
      hostname,
