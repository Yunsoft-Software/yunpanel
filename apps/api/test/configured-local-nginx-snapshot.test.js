...
test('configured local runtime enriches inventory with the legacy-compatible Nginx snapshot', async () => {
  let startOptions;
  ...
  assert.deepEqual(await startOptions.snapshotProvider(), {
    inventory: {
      hostname: 'host-1.example.local',
      mode: 'local',
      nginx: { available: true, version: 'nginx/1.24.0', config: { prefix: '/etc/nginx' } },
    },
  });
});
