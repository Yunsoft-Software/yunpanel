...
function fixture(options = {}) {
  ...
  const run = async (method, path = '/api/hosting-accounts', body = {}) => {
...
test('route family matching captures unknown children without matching another namespace', () => {
  for (const path of ['/api/hosting-accounts', '/api/hosting-accounts/a/transfer', '/api/hosting-accounts/']) assert.equal(isHostingAccountPath(path), true);
  for (const path of [null, {}, '/api/hosting-accounts-else', '/api/hosting-accountsX']) assert.equal(isHostingAccountPath(path), false);
});
...
test('GET list delegates filtered query; item GET delegates the exact ID', async () => {
  const f = fixture();
  assert.equal((await f.run('GET', '/api/hosting-accounts?kind=customer&direct=true')).status, 200);
  assert.deepEqual(f.calls[0], { name: 'list', args: [{ kind: 'customer', resellerId: null }] });
  await f.run('GET', '/api/hosting-accounts/customer-a');
...
test('PATCH limits keeps revision and fields for the existing store to validate', async () => {
  ...
  const result = await f.run('PATCH', '/api/hosting-accounts/reseller-a/limits', body);
...
test('profile removal requires bound confirmation and does not claim login deletion', async () => {
  const f = fixture(); const result = await f.run('DELETE', '/api/hosting-accounts/customer-a/profile', { revision: 2, confirmation: 'unregister-hosting-profile:customer-a:2' });
...
for (const body of [{ revision: 2 }, { revision: 2, confirmation: 'unregister-hosting-profile:other:2' },
  ...
  test(`rejects invalid unregister confirmation ${JSON.stringify(body)}`, async () => {
    const f = fixture(); await assert.rejects(f.run('DELETE', '/api/hosting-accounts/customer-a/profile', body), code('hosting_profile_confirmation_required')); assert.equal(f.calls.length, 0);
  });
}
for (const [method, path, expected] of [['DELETE', '/api/hosting-accounts/a', 'method_not_allowed'],
  ['POST', '/api/hosting-accounts/a/limits', 'method_not_allowed'], ['PATCH', '/api/hosting-accounts/a/profile', 'method_not_allowed'],
  ['GET', '/api/hosting-accounts/a/transfer', 'not_found'], ['POST', '/api/hosting-accounts/a/sites', 'not_found'],
  ['GET', '/api/hosting-accounts/a%2Fb', 'not_found'], ['HEAD', '/api/hosting-accounts', 'method_not_allowed']]) {
...
test('non-list requests reject query modifiers', async () => {
  const f = fixture(); await assert.rejects(f.run('POST', '/api/hosting-accounts?kind=customer'), code('invalid_hosting_account_query'));
  await assert.rejects(f.run('GET', '/api/hosting-accounts/a?direct=true'), code('invalid_hosting_account_query'));
  assert.equal(f.calls.length, 0);
});
