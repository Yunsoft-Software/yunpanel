...
async function recorderFixture() {
  ...
  await startConfiguredLocalRuntime({
    ...
    createNodeRestartReceipts: () => ({ async write(value) { writes.push(value); } }),
...
test('configured runtime records only exact successful Node restart result', async () => {
  ...
  await fx.recorder({
    ...
    payload: { applicationId, releaseId, runtime: { port: 3100, healthPath: '/health' } },
    result,
  });
...
test('mismatched Node restart result is never persisted as recovery evidence', async () => {
  const fx = await recorderFixture();
  await assert.rejects(
    fx.recorder({
      ...
      operation: OPERATIONS.APP_NODE_RESTART,
      payload: { applicationId, releaseId, runtime: { port: 3100, healthPath: '/health' } },
      result: { releaseId, serviceName: 'yunpanel-node-0123456789abcdef.service', port: 3200, healthPath: '/health', healthy: true, restarted: true },
    }),
