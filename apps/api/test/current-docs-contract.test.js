...
const currentDocs = [
  ...
  'docs/history/passenger-provisioning-live-acceptance-2026-09-19.md',
...
test('current docs expose the implemented agentless migration and deferred-design boundaries', async () => {
  ...
  assert.doesNotMatch(docs.get('todo.md'), /Yeni durable Website provisioning store ile API restart testi yap/);
  assert.match(
    docs.get('docs/history/passenger-provisioning-live-acceptance-2026-09-19.md'),
  ...
  assert.match(docs.get('docs/website-workspace.md'), /common audit metadata/);
  assert.match(docs.get('docs/website-workspace.md'), /Owner-only/);
