...
export default function WebsiteIsolationPanel({ websiteId }) {
  ...
  >
    ...
    {audit?.migrationRequired && <div className="ws-section-body"><div className="ws-notice ws-notice-warn"><div><strong>{audit.migration?.applyAvailable ? (identityApplyAvailable ? 'Unix identity migration uygulanabilir' : sftpApplyAvailable ? 'SFTP migration uygulanabilir' : phpApplyAvailable ? 'PHP-FPM pool migration uygulanabilir' : 'Workspace migration uygulanabilir') : 'Bu drift için migration apply kapalı'}</strong><p>{audit.migration?.warning}</p>{audit.migration?.applyAvailable && <Button variant="primary" disabled={busy} onClick={() => setConfirmation({ kind: 'apply' })}>Exact değişiklikleri uygula</Button>}</div></div></div>}
