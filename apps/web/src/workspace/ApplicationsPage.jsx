...
function ApplicationInventory() {
  ...
  const query = params.get('q') ?? ''; const serverId = params.get('server') ?? '';
  const items = applications.items.filter((app) => (!serverId || app.serverId === serverId) && app.name.toLowerCase().includes(query.toLowerCase()));
  ...
  return <><PageHeading title="Uygulamalar" description="Mevcut Node.js ve statik uygulama kayıtları; site bağlantıları Web Siteleri bölümündedir." actions={<LinkButton to="/applications/new" variant="primary" icon="plus">Uygulama ekle</LinkButton>} /><ErrorNotice error={error} /><Section title="Uygulama envanteri"><div className="ws-filters"><label className="ws-filter-search">Uygulama ara<input type="search" value={query} onChange={(event) => filter('q', event.target.value)} /></label><label>Sunucu<select value={serverId} onChange={(event) => filter('server', event.target.value)}><option value="">Tüm sunucular</option>{servers.items.map((item) => <option key={item.id} value={item.id}>{item.displayName ?? item.name ?? item.hostname}</option>)}</select></label></div><CollectionNotice resource={applications} label="Uygulamalar" />
    ...
    })}</tbody></table></div> : applications.status === 'ready' && <EmptyState title="Uygulama bulunamadı" detail="Yeni uygulama oluşturun veya arama ve sunucu filtresini değiştirin." icon="code" />}
...
function NewApplication() {
  ...
  const [form, setForm] = useState({ type: 'node', serverId: '', name: '', repositoryUrl: '', branch: 'main', port: '', entryFile: 'server.js', healthPath: '/health', outputDir: 'dist' });
