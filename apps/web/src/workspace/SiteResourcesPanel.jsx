...
export default function SiteResourcesPanel({ domain, website, application, server, activeTab }) {
  ...
  const mailSection = (
    <Section title="Mail & Webmail" description="Web Domain kimliğiyle explicit bağlı mail-domain kayıtları ve Roundcube webmail erişimi." actions={<Button icon="refresh" disabled={busy} onClick={load}>Yenile</Button>}>
      {mails.length ? <div className="ws-table-scroll"><table className="ws-table"><thead><tr><th>Domain</th><th>Mod</th><th>Durum</th><th className="ws-row-end">Webmail</th></tr></thead><tbody>{mails.map((mail) => <tr key={mail.id}><td><strong>{mail.domainName}</strong></td><td>{mail.managementMode}</td><td><Badge state={mail.status === 'enabled' || mail.status === 'ready' ? 'active' : mail.status === 'degraded' ? 'warning' : 'offline'}>{mail.status}</Badge></td><td className="ws-row-end"><a className="ws-button ws-button-secondary" href={`https://webmail.${mail.domainName}`} target="_blank" rel="noreferrer">Webmail’e Git ↗</a></td></tr>)}</tbody></table></div> : mailDomains !== undefined && <EmptyState icon="mail" title="Bağlı mail domain yok" detail="Bu web Domain için explicit mail-domain lifecycle kaydı bulunmuyor." />}
      <div className="ws-section-body ws-actions">{mails[0] ? <LinkButton to={`/mail/${encodeURIComponent(mails[0].id)}`} icon="mail">Mail’i yönet</LinkButton> : <LinkButton to="/mail" icon="mail">Mail domainleri</LinkButton>}{mails[0] && <a className="ws-button ws-button-primary" href={`https://webmail.${mails[0].domainName}`} target="_blank" rel="noreferrer">Webmail (Roundcube) ↗</a>}</div>
    </Section>
  );
