...
const navigation = [
  ...
  ['/databases', 'Veritabanları', 'database'], ['/docker', 'Docker', 'box'], ['/mail', 'Mail', 'mail'], ['/jobs', 'İşler', 'jobs'], ['/audit', 'Denetim', 'shield'], ['/settings', 'Ayarlar', 'settings'],
];
...
function Shell() {
  const { domains, jobs, notice, setNotice, canManage } = useWorkspace();
  ...
  const websiteCount = knownCount(domains); const jobCount = canManage ? knownCount(jobs, (job) => ['queued', 'running'].includes(job.status)) : null;
  const visibleNavigation = canManage ? navigation : navigation.filter(([to]) => readOnlyNavigation.has(to));
  useEffect(() => {
  ...
  return <div className="workspace-shell">
    ...
    <aside ref={menu} id="workspace-navigation" className={`ws-sidebar ${menuOpen ? 'is-open' : ''}`} inert={narrow && !menuOpen} aria-label="Ana menü" role={narrow && menuOpen ? 'dialog' : undefined} aria-modal={narrow && menuOpen ? true : undefined}>
      ...
      <p className="ws-nav-label">YÖNETİM</p><nav className="ws-nav" aria-label="Panel bölümleri">{visibleNavigation.map(([to, label, icon]) => <NavLink key={to} to={to}><Icon name={icon} /><span>{label}</span>{to === '/websites' && websiteCount !== null && <span className="ws-nav-count">{websiteCount}</span>}{to === '/jobs' && jobCount > 0 && <span className="ws-nav-count">{jobCount}</span>}</NavLink>)}</nav>
      <div className="ws-sidebar-footer"><strong>Yunsoft / YunPanel</strong><span>{canManage ? <>Veriler 15 saniyede bir yenilenir.<br />İşlem sonuçları ayrıca izlenir.</> : 'Read Only görünümünde yalnız izinli envanter verileri gösterilir.'}</span></div>
    </aside>
