...
export function removalOperation(value,scope){
 ...
 return Object.freeze({id:value.id,status:value.status,updatedAt:value.updatedAt??null,steps:Object.freeze(value.steps.map((step)=>{
...
export function removalErrorMessage(error){return ({website_removal_preview_stale:'Site durumu değişti. Silme etkisini yeniden yükleyin.',site_scope_forbidden:'Bu siteye erişim izniniz yok.'})[error?.code]??'Site silme bilgileri alınamadı.';}
