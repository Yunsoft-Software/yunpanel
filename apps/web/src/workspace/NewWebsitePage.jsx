...
import { availableExistingApplications, existingApplicationType, siteCreateInputFromForm } from './new-website-form.js';
...
function WebsiteForm({ parentId }) {
  ...
  const locked = baseLocked || Boolean(existingType && (applications.status !== 'ready' || websites.status !== 'ready'));
  function update(key, value) { setDirty(true); setForm((current) => ({ ...current, [key]: value, ...(['parentDomainId', 'mode', 'sourceMode'].includes(key) ? { applicationId: '' } : {}) })); }
  ...
  return <>
    ...
    <PageHeading title={form.mode === 'subdomain' ? 'Alt alan adı ekle' : 'Web sitesi ekle'} description="Ayrı bir uygulama oluşturun veya kullanılmamış bir uygulamayı yeni Website kimliğine bağlayın." />
    {created ? <Section title="Site kaydı oluşturuldu"><EmptyState icon="check" title={created.primaryDomain} detail="Kayıt taslak olarak oluşturuldu. DNS kayıtlarını hazırlayın; ardından Nginx yapılandırmasını ve SSL’i site içinden etkinleştirin." action={<LinkButton variant="primary" icon="arrow" to={siteHref(created.id, 'domains')}>Siteyi yapılandır</LinkButton>} /></Section> : <Section title="Site yapılandırması" description="Bağımsız alan adı ve alt alan adı varsayılan olarak ayrı Website, Application ve Unix kimliği alır.">
      <CollectionNotice resource={domains} label="Alan adları" /><CollectionNotice resource={servers} label="Yerel sunucu" />{existingType && <><CollectionNotice resource={applications} label="Uygulamalar" /><CollectionNotice resource={websites} label="Website bağları" /></>}
      <form className="ws-form" onSubmit={submit}><ErrorNotice error={error} /><fieldset disabled={baseLocked}>
        ...
        </div>{existingType && !eligible.length && applications.status === 'ready' && websites.status === 'ready' && <p className="ws-muted">Bu sunucuda kullanılmamış uygun uygulama bulunmuyor. Yeni Application oluşturma seçeneklerinden birini kullanın.</p>}</div>
        ...
        <footer className="ws-form-footer" style={{ marginTop: 24 }}><LinkButton to="/websites">Vazgeç</LinkButton><Button type="submit" variant="primary" icon="plus" disabled={locked || !serverId || (existingType && !form.applicationId)}>{busy ? 'Oluşturuluyor…' : 'Siteyi oluştur'}</Button></footer>
      </fieldset></form>
    </Section>}
  </>;
}
