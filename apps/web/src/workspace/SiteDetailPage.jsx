...
function SiteWorkspace({ websiteId, tab }) {
  ...
  return <>
    ...
    <div className="ws-site-meta"><Badge state={domain.state} /><Badge state={ssl.state}>{ssl.label}</Badge><span>{application ? `${application.name} · ${application.type === 'node' ? `Node.js ${application.runtime?.nodeMajor ?? ''}` : 'Statik uygulama'}` : website?.runtimeType === 'docker' ? 'Managed Docker / proxy' : domain.targetType === 'static' ? 'Statik dosya yayını' : `Proxy · 127.0.0.1:${domain.target?.upstreamPort ?? '—'}`}</span></div>
