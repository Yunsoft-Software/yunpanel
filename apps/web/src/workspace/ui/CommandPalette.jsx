...
export default function CommandPalette({ domains, canManage, isOwner = false, isReseller = false, onClose }) {
  const [query, setQuery] = useState('');
  const [index, setIndex] = useState(0);
  const id = useId();
  const navigate = useNavigate();
  const entries = useMemo(() => commandEntries({ query, canManage, isOwner, isReseller, domains }), [query, canManage, isOwner, isReseller, domains]);
