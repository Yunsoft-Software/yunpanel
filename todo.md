...
- [ ] Static deployment source testlerinde `website-static-deployment-manager`, `static-deployment-legacy-fallback`, `static-deployment-router` ve `static-deployment-root-routing` birlikte geçsin: canonical identity önce seçilsin; yalnız önceden var olan exact `yunapp-*` + group + HOME=`/var/lib/yunpanel/build/<applicationId>` legacy fallback'e uygun sayılsın; eksik/mismatched identity yeni user yaratmasın; fallback içinde `/usr/sbin/useradd` fiziksel olarak reddedilsin; canonical deploy `runuser` HOME'unu `/var/lib/yunpanel/data/<applicationId>` olarak sabitlesin. Retained agent'ın explicit legacy subpath testi de regresyon vermesin.
...
- [ ] Test hostunda canonical static Website deploy'un mevcut provisioned `yunapp-*` hesabını kullandığını ve hesabın HOME'unu `/var/lib/yunpanel/build/<applicationId>` olarak değiştirmediğini doğrula. Ayrı migration fixture'ında yalnız önceden var olan HOME=`/var/lib/yunpanel/build/<applicationId>` legacy hesabı fallback ile deploy edebilsin; hesap yoksa, group/home drift varsa veya deploy sırasında identity kaybolursa `useradd` çalışmasın ve işlem fail-closed kalsın. `/var/lib/yunpanel/backups/resources` site user'a chown edilmesin ve root/control-plane private scope olarak kalsın.
...
- [ ] Static Website gerçek provisioned site user altında build/deploy etsin; process environment `HOME=/var/lib/yunpanel/data/<applicationId>` olsun, build workspace ayrı `/var/lib/yunpanel/build/<applicationId>` altında kalabilsin ve deploy motoru ikinci Unix identity yaratmasın. Eski build-home identity yalnız kanıtlı migration fallback olarak kabul edilsin; fresh/unproven identity için fallback user yaratmasın.
