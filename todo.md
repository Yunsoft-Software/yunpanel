...
2026-09-09'daki birleşik ağaç Node 24.19 üzerinde yerelde ve Node 24.20 Ubuntu package build hostunda filtresiz doğrulanmıştı: repository policy, **473 test** ve Vite production build geçti. Bu tarihsel kabul daha sonra eklenen agentless local-runtime, complete local snapshots, managed-service/DB management, durable recovery, fresh local bootstrap, operation-specific running recovery, Node/static recovery receipts, system-upgrade recovery, certificate recovery, agentless default dev akışı ve enrollment UI temizliğini kapsamaz. Güncel `main` ayrıca çalıştırılmadan “full check geçti” denmeyecek.
...
- [ ] **Güncel `main`** üzerinde Node 24.11.1+ ortamında temiz install sonrası filtresiz `npm run check` ve production Vite build'i çalıştır. Agentless runtime/snapshot, migration, durable recovery, read-only payload recovery, domain/static/Node/DB/service/system-upgrade/certificate recovery, private receipt/evidence ve ilgili guard testlerinin tamamı dahil olmalı.
...
- [ ] Güncel source'u yeni `.deb` adayına dönüştürmeden önce Node 24 full check'i bitir; ardından root `yunpanel-api.service`, local-runtime `create/status/bind/release`, bütün güncel `job-recovery` komutları, recovery sidecar, private receipt/evidence store'ları, complete local snapshots, web secret sandbox, auth ownership migration, managed-service ve DB queue/API dosyalarının pakete girdiğini `dpkg-deb -c/-I` ile doğrula.
- [ ] Yeni agentless aday `.deb` install/upgrade sırasında auth DB/master key/state ownership, API root service, web sandbox, disabled-agent preservation, fresh credentialless local create, legacy migration rollback yolu, durable recovery komutları, private receipt modes ve restart davranışını izole test hostunda doğrula; mevcut `0.3.0-4` canlı hosta kör deploy yapma.
