...
Güncel `main` ve iki plan dosyasını oku. Kullanıcı ayrıca istemedikçe yeni branch oluşturma; doğrudan main'de küçük commitler kullan. GitHub Actions kullanma. Bu ortamda canlı panel/SSH/deployment yapılmadı. Routed UI'nin kapsamı `docs/website-workspace.md`, veri yükleme/görünüm değişiklikleri `docs/ui-runtime.md`, kullanıcı yönetimi ve bu incrementin test sınırları `docs/user-administration.md` içinde; auth/MFA/parent modelinin sınırları ilgili runbook'lardadır. Kodun repoda bulunması native/tam workspace/tarayıcı kabulü veya Website migration, agentless backend ve terminalin tamamlandığı anlamına gelmez.

## T-USER — P0: Kullanıcı yönetimi native entegrasyon ve gerçek kabul

- [ ] Tam repo ve Node 24.11.1+ / npm 11+ ile `user-admin-native.test.js` içindeki üç testi, 37 odaklı store/HTTP/client testini ve mevcut auth/MFA/gateway/core testlerini birlikte çalıştır; ardından filtresiz `npm run check` ve production build yap. Bu incrementin 37 testi Node 22.16.0 kaynak alt kümesinde gerçek SQLite/HTTP ve kontrollü parola/session adapter'larıyla geçti; native Argon2, gerçek MFA login, React render veya full-workspace sonucu değildir. Native deneme tam kaynak/bağımlılık ve desteklenen runtime olmadan çalışamadı; uygulama runtime gereksinimini düşürme.
- [ ] Gerçek `index.js -> authenticated API -> core/domain` ve web gateway zincirinde `/api/panel/users` CRUD'u doğrula. Raw API'nin `/api/users` sınırı, gateway'in raw route reddi, anonymous/Read Only/unenrolled Owner reddi, CSRF/Origin ve body limitleri korunmalı. Kullanıcı yönetimi `/api/auth/*` self-service istisnasına taşınmamalı.
- [ ] Ayrı SQLite bağlantıları/process'leriyle eşzamanlı son-Owner disable/demotion/delete, username çakışması ve eski revision edit/delete yarışlarını test et. Native hashing sırasında actor logout/demotion/MFA reset ve hedef account değişirken in-flight password login'i dene. Transaction/audit başarısızlığında hesap ve revocation yarım kalmamalı; birden fazla aktif Owner aynı anda ortadan kaldırılamamalı.
- [ ] Mevcut auth v2 kopyasında additive user-admin sidecar v1 migration'ı, tekrar startup ve unknown-version reddini test et. Kullanıcı/parola/session/secret ilişkileri korunmalı; tutarlı SQLite yedeği/restore ve eski paket davranışı ayrıca doğrulanmalı. PRAGMA sürümünü düşürme veya çalışan DB'den sidecar silme; eski kodun yeni login-revision kontrolünü uygulamadığını hesaba kat.
- [ ] Gerçek tarayıcıda Ayarlar → Kullanıcılar (`/settings/users`) için oluşturma, username/rol/aktivite düzenleme, etkisi açık typed-confirm silme, son Owner hatası, 25'ten fazla hesap ve son sayfadan silmeyi dört ekran boyutunda test et. Klavye/focus/Escape, dirty form, back/forward/reload, çift submit, gecikmiş/bozuk yanıt ve network kopmasını kapsa. Belirsiz mutation sonucunda kör retry yok; form kapanıp liste kontrol edilmeli. Sayfa değişiminin ilk render'ında önceki sayfa kayıtları kullanılmamalı.
- [ ] Değişiklik sonrası hedef session/pending enrollment/login challenge iptalini gerçek MFA ile doğrula; edit kayıtlı factor'ü korumalı, delete factor/recovery'yi de silmeli. Kendi hesabı değişince workspace kapanmalı; eski 200/401 yanıt yeni login cookie'sini silmemeli. Read Only yönetim verisi alamamalı; parola/hash/token/MFA secret liste, audit, URL/localStorage veya telemetry'ye düşmemeli. Bu kabul bitmeden production kullanıcı yönetimini doğrulanmış sayma.

## T-UI — P1: Yeni routed arayüzün gerçek kabulü

- [ ] Tam repo üzerinde Node 24.11.1+ / npm 11+ ile bağımlılıkları kur; yeni `react-router@8.3.0` dahil manifest çözümünü doğrula. Filtre olmadan `npm run check` ve Vite production build çalıştır. Önceki küçük kaynak alt-kümesi testlerini tam workspace/build kabulü sayma.
- [ ] Built React uygulamasını gerçek tarayıcıda aç. Önceki yerel preview `ERR_BLOCKED_BY_ADMINISTRATOR` nedeniyle render etmedi; gerçek ekran görüntüsü, mobil/focus veya gezinme kabulü yapılmış değil. JSX transpilation/CSS parse sonucunu tasarım kabulü sayma.
...
- [ ] `/dashboard`, `/websites`, `/websites/new`, `/websites/:id/:tab`, `/applications`, `/applications/new`, `/domains`, `/servers`, `/jobs`, `/settings`, `/settings/users` için doğrudan URL, reload, back/forward ve yanlış route testlerini yap. HTTPS reverse proxy SPA fallback ve statik assetler API auth'unu atlamadan çalışmalı. Domain ID kullanan mevcut route'u kalıcı Website migration kabulü sayma.
...
- [ ] 401/403/404/409 ve network/missing-data hallerini her koleksiyonda oluştur. Yetki kaybında cache silinmeli; sadece network hatasında tarihli stale görünüm kalmalı. Tek tablo hatası diğer veriyi silmemeli; bilinmeyen ölçüm sıfır/yanlış yeşil SSL olmamalı. Tüm bunları actual React etkileşimiyle kontrol et. Kullanıcı yönetimi listesi daha sıkıdır: network hatasında da eski hesap verisini göstermez.
- [ ] 1440×900, 1920×1080, 1280×800 ve 390×844 boyutlarında Dashboard/Websites/site Node/SSL/forms/jobs/kullanıcı ekranlarını incele. Desktop sidebar, mobil focus trap/Escape/inert/backdrop, Cmd/Ctrl+K arama, skip link, modal focus restore, contrast/okunurluk, uzun tablolar ve taşmayı düzelt. MFA/enrollment ekranında sidebar yokken hesap çubuğu boşluk bırakmamalı.
...
- [ ] `docs/ui-runtime.md` içindeki beş dosyanın 40 testini desteklenen Node 24.11.1+ ortamında, ardından tam workspace/build ve gerçek tarayıcıyla doğrula. Bu modülün önceki testi Node 22.16.0 kaynak alt kümesiyle çalıştı; 28 yeni kontrol + 12 mevcut model testi, React render/HTTP/kripto kabulü değildir.
...
- [ ] Rate limit, yanlış parola, idle/absolute, refresh/logout, password/role/user-disable senaryolarını test et. Son Owner ve Read Only mutasyon reddini mevcut user-admin API'siyle doğrula; terminal ve diğer canlı bağlantılar eklendiğinde aynı red/yetki iptali testini genişlet.
