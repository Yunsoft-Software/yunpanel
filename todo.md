...
- [ ] Static Website deployment source testlerinde eksik Unix user veya HOME/group drift olduğunda deploy başlamadan fail-closed kalındığını; wrapper'ın `/usr/sbin/useradd` çağrısını reddettiğini; `runuser` HOME'unu canonical site HOME'una sabitlediğini ve iki farklı application'ın paralel deploy'unda HOME/context çaprazlanmadığını doğrula.
...
- [ ] Test hostunda static Website deploy'un mevcut provisioned `yunapp-*` hesabını kullandığını ve hesabın HOME'unu `/var/lib/yunpanel/build/<applicationId>` olarak değiştirmediğini doğrula. Deploy sırasında user silinir/drift oluşursa yanlış HOME'la yeni kullanıcı yaratılmasın; işlem fail-closed kalsın. `/var/lib/yunpanel/backups/resources` site user'a chown edilmesin ve root/control-plane private scope olarak kalsın.
...
- [ ] Static Website gerçek provisioned site user altında build/deploy etsin; process environment `HOME=/var/lib/yunpanel/data/<applicationId>` olsun, build workspace ayrı `/var/lib/yunpanel/build/<applicationId>` altında kalabilsin ve deploy motoru ikinci Unix identity yaratmasın.
