...
- [ ] Production package `YUNPANEL_WEBSITE_STORE=/var/lib/yunpanel/control-plane/website-registry.json`, `YUNPANEL_WEBSITE_MIGRATION_POLICY_STORE=/var/lib/yunpanel/control-plane/website-migration-policy.json`, `YUNPANEL_DNS_HOSTING_STORE=/var/lib/yunpanel/control-plane/dns-hosting-registry.json` ve `YUNPANEL_MAIL_DOMAIN_STORE=/var/lib/yunpanel/control-plane/mail-domain-registry.json` altında private state oluştursun; restart/upgrade kimlikleri, explicit ilişkileri veya policy mode/digest bilgisini değiştirmesin.
...
- [ ] Owner Website/Domain move-delete impact preview'ını gerçek packaged listener'da doğrula: child/linked Domain, Application, explicit DNS-zone/mail-domain, certificate ve queued/running job kimlikleri bounded olsun; mailbox/backup/cron/Docker association yokken unavailable blocker ve `safeToApply=false` dönsün. Read Only iki POST route'unda 403 kalsın; apply/cascade rotası bulunmasın.
