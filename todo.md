...
- [ ] Zone/RRset create-update-delete/no-op; SOA serial, NS, A/AAAA/CNAME/MX/TXT/CAA/SRV, TTL, wildcard, IDN ve DNSSEC API üzerinden gerçek authoritative cevapla doğrulansın. Enabled local-mail zone re-apply shared mail identity MX, IMAP/submission SRV ve current+pending-retirement DKIM TXT kayıtlarını authoritative cevapta üretsin; disabled local-mail re-apply yalnız YunPanel `mail` ownership kayıtlarını kaldırsın, manual kayıtları korusun ve webmail/autodiscover endpoint'i hazır değilken dead kayıt yayınlamasın.
