...
- [ ] PowerDNS explicit rollback gerçek hostta previous config + authoritative receipt'i byte/uid/gid/mode olarak restore etsin; configtest/restart/API health sonrası exact previous topology doğrulansın. Rollback health failure enjekte edildiğinde current config/receipt geri gelsin ve current intent yeniden healthy olsun; compensation failure başarılı rollback gibi raporlanmasın.
