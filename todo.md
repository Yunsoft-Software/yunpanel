...
2026-09-09'daki birleşik ağaç Node 24.19 üzerinde yerelde ve Node 24.20 Ubuntu package build hostunda filtresiz doğrulanmıştı: repository policy, **473 test** ve Vite production build geçti. Bu tarihsel kabul daha sonra eklenen agentless local-runtime, complete local snapshots, managed-service/DB management, durable recovery, fresh local bootstrap, read-only running recovery, verified `domain.stage` recovery, static deployment receipt/evidence recovery, agentless default dev akışı ve enrollment UI temizliğini kapsamaz. Güncel `main` ayrıca çalıştırılmadan “full check geçti” denmeyecek.
...
- [ ] **Güncel `main`** üzerinde Node 24.11.1+ ortamında temiz install sonrası filtresiz `npm run check` ve production Vite build'i çalıştır. Agentless runtime/snapshot, migration, durable recovery, verified domain/static recovery, static receipt/evidence, managed-service, DB ve yeni UI guard testlerinin tamamı dahil olmalı.
...
- [ ] `recover-readonly <server> <job> --confirm` gerçek test hostunda yalnız payload'sız `system.packages.inspect` ve `database.inspect` running joblarını kabul etmeli. Exact server hostname uyuşmazlığı, host probe hatası veya durable completion belirsizliğinde job/recovery unresolved kalmalı; mutation operasyonu bu komuta girmemeli.
- [ ] `recover-domain-stage <server> <job> --confirm` için exact persisted job + exact local hostname + Nginx staged config render/checksum kanıtını gerçek filesystem ile doğrula. Evidence yok/drift varsa hiçbir journal/completion/resource mutation başlamamalı; evidence varsa job terminal + domain `staged` + recovery clear olmalı. `domain.activate` bu yoldan reddedilmeli.
- [ ] Static deploy recovery receipt dizinlerini gerçek packaged hostta doğrula: `/var/lib/yunpanel/recovery/static-deployments` ve app alt dizinleri 0700, receipt 0600 olmalı; receipt generic payload/result/secret alanı kabul etmemeli. Başarılı yeni static deploy receipt yazamasa bile deploy sonucu yanlışlıkla failed olmamalı.
- [ ] `recover-static-deploy <server> <job> --confirm` için **receipt + `current -> releases/<job-id>` symlink + gerçek release directory** üçlü kanıtını doğrula. Receipt stale/missing, current başka release, symlink release veya application/job identity drift varsa job/application mutation başlamamalı; exact kanıtta aynı job terminal olmalı, application `active` olmalı ve recovery journal kapanmalı.
- [ ] Şu running mutationların recovery komutları olmadığını ve generic `force-success` bulunmadığını doğrula: `domain.activate`, static rollback, Node deploy/rollback/restart, package upgrade, managed-service install/control, DB create/delete, SSL issue/renew. Dış host kanıtı olmadan bunlara outcome atanamamalı.
- [ ] Fresh agentless fixture'da packaged `local-runtime create --confirm` çalıştır: API+agent durmuş ve queued/running/recovery işi yokken OS hostname için `executionMode=local`, `agentTokenHash=null`, `enrolledAt=null` server kaydı oluşmalı; exact çıktı `YUNPANEL_LOCAL_SERVER_ID` olarak kullanılmalı. Bu local-only kayıt `release --confirm` ile agent moduna geçirilememeli (`agent_credentials_unavailable`).
...
- [ ] Güncel server UI'da enrollment-token formunun bulunmadığını doğrula. Server yokken agentless `local-runtime create --confirm` bootstrap yönlendirmesi gösterilmeli ve browser `/servers/enrollment-tokens` isteği üretmemeli.
...
- [ ] Güncel source'u yeni `.deb` adayına dönüştürmeden önce Node 24 full check'i bitir; ardından root `yunpanel-api.service`, local-runtime `create/status/bind/release`, `job-recovery status/reconcile/recover-readonly/recover-domain-stage/recover-static-deploy`, recovery sidecar, static receipt/evidence, complete local snapshots, web secret sandbox, auth ownership migration, managed-service ve DB queue/API/UI dosyalarının pakete girdiğini `dpkg-deb -c/-I` ile doğrula.
- [ ] Yeni agentless aday `.deb` install/upgrade sırasında auth DB/master key/state ownership, API root service, web sandbox, disabled-agent preservation, fresh credentialless local create, legacy migration rollback yolu, durable recovery komutları, private static receipt modes ve restart davranışını izole test hostunda doğrula; mevcut `0.3.0-4` canlı hosta kör deploy yapma.
...
- [ ] Mevcut enroll edilmiş host migration'ında packaged `local-runtime.mjs` + `docs/local-runtime-migration.md` sırasını gerçek test paketinde uygula: verified backup -> job drain/recovery clear -> API+agent stop -> status preflight -> exact UUID/hostname bind -> `YUNPANEL_LOCAL_SERVER_ID` config -> agent disable -> local API start/health. Server/job state paths `/var/lib/yunpanel/control-plane` altında olmalı; IDs, auth DB, master key, vhost, cert, release ve users korunmalı.
- [ ] Fresh agentless package kurulumunda API+agent durmuş ve queue/recovery boşken `local-runtime create --confirm` çalıştır. Enrollment token/agent token veya `/etc/yunpanel/agent/agent.env` gerekmemeli; server state `local`, `agentTokenHash=null`, `enrolledAt=null` olmalı. Exact ID `api.env` içine yazıldıktan sonra local API health + inventory/services/Docker/Nginx snapshot çalışmalı ve legacy transport aynı ID için kullanılamamalı.
- [ ] Enroll edilmiş test hostunda rollback runbook'unu uygula: local jobs drain/recovery clear -> API+agent stop -> env gate kaldır -> `release --confirm` -> agent enable/start. Bind/release sırasında active servis, queued/running job, recovery sidecar, hostname mismatch, outside/relative state path veya unreadable systemd state fail-closed kalmalı. Fresh local-only kimlikte `release --confirm` reddedilmeli; sahte credential üretilmemeli.
- [ ] Recovery tatbikatı yap: acknowledgement/reconciliation kesintisiyle recovery sidecar üret, restart sonrası korunduğunu göster. Terminal jobda `reconcile` yalnız resource state uygulasın. Running read-only inspection için `recover-readonly`; exact Nginx staged evidence için `recover-domain-stage`; static receipt/current-release evidence için `recover-static-deploy` yalnız kendi allowlist'lerinde çalışsın. Evidence yoksa journal temizlenmemeli. Kalan unsupported mutationlarda CLI outcome atamayı reddetmeli.
- [ ] Static deploy migration/recovery tatbikatında başarılı local deploy sonrası private receipt'in yazıldığını; receipt kaybolursa normal deploy'un başarılı kalıp yalnız crash-recovery kanıtının kaybolduğunu doğrula. Stale receipt current release'i geriye çekmemeli.
- [ ] Package upgrade öncesi disabled `yun-agent.service` sonrasında da disabled kalmalı; fresh install yalnız `/etc/yunpanel/agent/agent.env` varsa legacy agent'ı enable etmeli. Fresh agentless kurulumda agent env/credential oluşturulmamalı.
