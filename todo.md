...
2026-09-09'daki birleşik ağaç Node 24.19 üzerinde yerelde ve Node 24.20 Ubuntu package build hostunda filtresiz doğrulanmıştı: repository policy, **473 test** ve Vite production build geçti. Bu tarihsel kabul daha sonra eklenen agentless local-runtime, managed-service, database management, durable recovery ve fresh local bootstrap commitlerini kapsamaz. Güncel source için aşağıdaki yeni Node 24/package/browser maddeleri ayrıca çalıştırılmadan “full check geçti” denmeyecek.
...
- [ ] Güncel `main` üzerinde agentless runtime + managed-service + DB queue/API/UI + migration + durable recovery + fresh local create değişiklikleriyle Node 24.11.1+ ortamında temiz install sonrası filtresiz `npm run check` ve production Vite build'i tekrar çalıştır. Yeni testleri atlama veya runtime gereksinimini düşürme.
...
- [ ] Gerçek job registry ve disk üzerinde claim/result yazımı, atomic rename, disk-full/read-only ve kaybolan acknowledgement hatalarını kontrollü üret. Başarılı host işi completion hatasında failed'e çevrilmemeli; belirsiz claim/complete/reconcile instance'ı durdurmalı ve yeni claim/otomatik retry olmamalı. Versioned `.recovery.json` sidecar 0600 kalmalı, ilk detection timestamp restartta korunmalı ve payload/result/error/secret taşımamalı.
- [ ] `job-recovery status` için gerçek persisted fixture'da running → `execution_state_unknown`, terminal journal → `reconciliation_required`, karışık set → `mixed_recovery_required` ayrımını doğrula. `reconcile <server> <job> --confirm` yalnız API+agent durmuşken terminal kaydı resource state'e tekrar uygulayıp journal'ı temizlemeli; running kaydı kesinlikle reddetmeli ve host operasyonunu tekrar çalıştırmamalı.
- [ ] Fresh agentless fixture'da packaged `local-runtime create --confirm` çalıştır: API+agent durmuş ve queued/running iş yokken OS hostname için `executionMode=local`, `agentTokenHash=null`, `enrolledAt=null` server kaydı oluşmalı; çıktıdaki exact ID `YUNPANEL_LOCAL_SERVER_ID` olarak kullanılmalı. Bu local-only kayıt `release --confirm` ile agent moduna geçirilememeli (`agent_credentials_unavailable`).
...
- [ ] Legacy agent + local worker dual-consumer testini gerçek durable queue üzerinde yap: local bind öncesi API ve agent durmuş + job drain şartı; bind sonrası aynı server kuyruğunu yalnız local executor tüketmeli. Local ownership altında legacy heartbeat/command/environment/result rotaları 409 `server_managed_locally` kalmalı. Node deploy/restart/rollback secret env değerleri job JSON'una düşmemeli.
...
- [ ] Güncel source'u yeni `.deb` adayına dönüştürmeden önce Node 24 full check'i bitir; ardından root `yunpanel-api.service`, local-runtime env/config/`create/status/bind/release` CLI, `job-recovery status/reconcile` CLI, recovery sidecar kodu, web secret sandbox, auth ownership migration, managed-service ve DB queue/API/UI dosyalarının pakete gerçekten girdiğini `dpkg-deb -c/-I` ile doğrula.
- [ ] Yeni agentless aday `.deb` için install/upgrade sırasında auth DB/master key/state ownership, API root service, web sandbox, agent enablement preservation, fresh credentialless local create, legacy migration CLI, durable recovery CLI ve restart davranışını izole test hostunda doğrula; mevcut `0.3.0-4` canlı hosta kör deploy yapma.
...
- [ ] Mevcut enroll edilmiş host migration'ında packaged `local-runtime.mjs` + `docs/local-runtime-migration.md` sırasını gerçek test paketinde uygula: verified backup -> job drain -> API+agent stop -> status preflight -> exact UUID/hostname bind -> `YUNPANEL_LOCAL_SERVER_ID` config -> agent disable -> local API start/health. Server/job state paths `/var/lib/yunpanel/control-plane` altında olmalı; IDs, auth DB, master key, vhost, cert, release ve users korunmalı.
- [ ] Fresh agentless package kurulumunda API+agent durmuş ve queue boşken `local-runtime create --confirm` çalıştır. Enrollment token/agent token veya `/etc/yunpanel/agent/agent.env` gerekmemeli; server state `local`, `agentTokenHash=null`, `enrolledAt=null` olmalı. Çıktıdaki exact ID `api.env` içine yazıldıktan sonra local API health/snapshot çalışmalı ve legacy transport aynı ID için kullanılamamalı.
- [ ] Enroll edilmiş test hostunda rollback runbook'unu uygula: local jobs drain -> API+agent stop -> env gate kaldır -> `release --confirm` -> agent enable/start. Bind/release sırasında active/activating/deactivating servis, queued/running job, hostname mismatch, outside/relative state path veya unreadable systemd state fail-closed kalmalı. Fresh local-only kimlikte agent credential olmadığı için `release --confirm` reddedilmeli; sahte credential üretilmemeli.
- [ ] Recovery tatbikatı yap: kontrollü acknowledgement/reconciliation kesintisiyle recovery sidecar üret, process restart sonrası kaydın korunduğunu göster. Terminal job için API+agent durmuşken `job-recovery reconcile ... --confirm` yalnız resource reconciliation yapmalı ve host işini tekrar çalıştırmamalı. Running/host sonucu belirsiz işte CLI mutation reddedilmeli; dış host kanıtı olmadan outcome atanıp journal temizlenmemeli.
